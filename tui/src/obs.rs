//! TUI observability wiring — JSON tracing parallel to the on-screen log pane.
//!
//! Fail-open: if the log dir is unwritable the TUI runs without JSON logs and
//! the in-app event log is unaffected. `RUST_LOG` overrides the default level.
//!
//! `FORGEISO_LOG_DIR` overrides the default `<state>/forgeiso/logs` directory.
...
pub fn init_tracing() -> Option<tracing_appender::non_blocking::WorkerGuard> {
    ...
    let appender = tracing_appender::rolling::daily(&log_dir, "forgeiso-tui.log");
