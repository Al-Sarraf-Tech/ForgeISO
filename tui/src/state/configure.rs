//! Step 2 (Configure) helpers on [`App`].
//!
//! Owns the per-tab field schema, the field-by-index getters/setters used by
//! the Configure tab UI, the form validator gate, and the `InjectConfig`
//! builder consumed by the build worker.

use forgeiso_engine::{
    ContainerConfig, FirewallConfig, GrubConfig, InjectConfig, IsoSource, NetworkConfig,
    ProxyConfig, SshConfig, SwapConfig, UserConfig,
};

use super::{App, ConfigTab, FieldDef};
