use std::path::PathBuf;
use std::sync::{mpsc, Arc};

use egui::{Color32, Frame, RichText, Stroke, Ui, Vec2};
use forgeiso_engine::{
    find_preset_by_str, resolve_url, AcquisitionStrategy, ContainerConfig, Distro, FirewallConfig,
    ForgeIsoEngine, GrubConfig, InjectConfig, IsoSource, NetworkConfig, ProxyConfig, SshConfig,
    SwapConfig, UserConfig,
};
use serde::{Deserialize, Serialize};

use crate::state::{
    lines, opt, BuildResult, DoctorReport, InjectState, Iso9660Compliance, LogEntry, LogLevel,
    PickTarget, StatusMsg, VerifyResult, VerifyState,
};
use crate::worker::{self, WorkerMsg};

// ── Persisted form state ───────────────────────────────────────────────────────

const STORAGE_KEY: &str = "forgeiso_v2";

#[derive(Default, Serialize, Deserialize)]
struct PersistedState {
    inject: InjectState,
    verify: VerifyState,
}

// ── Palette ───────────────────────────────────────────────────────────────────

const BG: Color32 = Color32::from_rgb(13, 17, 23);
const SURFACE: Color32 = Color32::from_rgb(22, 27, 34);
const PANEL_BG: Color32 = Color32::from_rgb(10, 14, 20);
const BORDER: Color32 = Color32::from_rgb(48, 54, 61);
const ACCENT: Color32 = Color32::from_rgb(47, 129, 247);
const GREEN: Color32 = Color32::from_rgb(63, 185, 80);
const RED: Color32 = Color32::from_rgb(248, 81, 73);
const AMBER: Color32 = Color32::from_rgb(210, 153, 34);
const TEXT: Color32 = Color32::from_rgb(230, 237, 243);
const MUTED: Color32 = Color32::from_rgb(139, 148, 158);
const TAB_ACTIVE: Color32 = Color32::from_rgb(33, 38, 45);
// Card-state background / border pairs
const CARD_GREEN_BG: Color32 = Color32::from_rgb(13, 28, 18);
const CARD_GREEN_BORDER: Color32 = Color32::from_rgb(40, 100, 55);
const CARD_RED_BG: Color32 = Color32::from_rgb(30, 15, 15);
const CARD_RED_BORDER: Color32 = Color32::from_rgb(100, 40, 40);
const CARD_AMBER_BG: Color32 = Color32::from_rgb(30, 24, 10);
const CARD_ORANGE_BG: Color32 = Color32::from_rgb(28, 18, 10);
const CARD_ORANGE_BORDER: Color32 = Color32::from_rgb(100, 60, 20);
// Log display
const LOG_TIMESTAMP: Color32 = Color32::from_rgb(70, 80, 95);
const LOG_PROGRESS: Color32 = Color32::from_rgb(80, 200, 120);
// ── UI helpers ────────────────────────────────────────────────────────────────

/// Thin muted label that sits above a field — slightly smaller than body text
/// so form structure is immediately obvious without visual noise.
fn lbl(ui: &mut Ui, text: &str) {
    ui.label(
        RichText::new(text)
            .size(12.0)
            .color(Color32::from_rgb(110, 120, 134)),
    );
    ui.add_space(2.0);
}

/// Section title with a 3-px accent bar on the left and a full-width hairline
/// below — gives each section a crisp, professional boundary.
fn section(ui: &mut Ui, text: &str) {
    ui.add_space(14.0);
    ui.horizontal(|ui| {
        let (rect, _) = ui.allocate_exact_size(Vec2::new(3.0, 16.0), egui::Sense::hover());
        ui.painter().rect_filled(rect, 2.0, ACCENT);
        ui.add_space(8.0);
        ui.label(
            RichText::new(text)
                .size(12.0)
                .strong()
                .color(Color32::from_rgb(180, 190, 205)),
        );
    });
    ui.add_space(5.0);
    // Full-width hairline under the section title
    let available = ui.available_width();
    let (line_rect, _) = ui.allocate_exact_size(Vec2::new(available, 1.0), egui::Sense::hover());
    ui.painter()
        .rect_filled(line_rect, 0.0, Color32::from_rgb(33, 38, 46));
    ui.add_space(8.0);
}

/// Thin horizontal rule — draws a 1px line in a slightly darker shade than BORDER
/// for a clean visual break without a harsh contrast jump.
fn rule(ui: &mut Ui) {
    ui.add_space(12.0);
    let w = ui.available_width();
    let (rect, _) = ui.allocate_exact_size(Vec2::new(w, 1.0), egui::Sense::hover());
    ui.painter()
        .rect_filled(rect, 0.0, Color32::from_rgb(33, 38, 46));
    ui.add_space(12.0);
}

/// Full-width primary action button with rounded corners.
fn action_btn(ui: &mut Ui, label: &str, enabled: bool) -> bool {
    let fill = if enabled { ACCENT } else { TAB_ACTIVE };
    let text_col = if enabled { Color32::WHITE } else { MUTED };
    let btn = egui::Button::new(RichText::new(label).size(15.0).strong().color(text_col))
        .fill(fill)
        .stroke(Stroke::new(1.0, if enabled { ACCENT } else { BORDER }))
        .corner_radius(8.0f32)
        .min_size(Vec2::new(ui.available_width(), 50.0));
    ui.add_enabled(enabled, btn).clicked()
}

/// Small "Browse" button that fits next to a text field.
fn browse_btn(ui: &mut Ui, enabled: bool) -> bool {
    let col = if enabled { TEXT } else { MUTED };
    ui.add_enabled(
        enabled,
        egui::Button::new(RichText::new("📂  Browse").size(13.0).color(col))
            .fill(SURFACE)
            .stroke(Stroke::new(1.0, BORDER))
            .corner_radius(6.0f32)
            .min_size(Vec2::new(90.0, 38.0)),
    )
    .clicked()
}

/// Small secondary button.
fn small_btn(ui: &mut Ui, label: &str, enabled: bool) -> bool {
    let col = if enabled { TEXT } else { MUTED };
    ui.add_enabled(
        enabled,
        egui::Button::new(RichText::new(label).size(13.0).color(col))
            .fill(SURFACE)
            .stroke(Stroke::new(1.0, BORDER))
            .corner_radius(6.0f32)
            .min_size(Vec2::new(100.0, 38.0)),
    )
    .clicked()
}

/// Coloured result card.
fn result_box(ui: &mut Ui, fill: Color32, border: Color32, add: impl FnOnce(&mut Ui)) {
    Frame::new()
        .fill(fill)
        .stroke(Stroke::new(1.0, border))
        .inner_margin(14.0f32)
        .corner_radius(6.0f32)
        .show(ui, add);
    ui.add_space(8.0);
}

/// Chip-style preset button — highlights with accent colour when `field` already
/// equals `value`, so the user can see what's currently selected at a glance.
/// Clicking it writes `value` into `field`; tooltip shows the full value.
fn chip_btn(ui: &mut Ui, label: &str, value: &str, field: &mut String, enabled: bool) {
    let active = field.trim() == value;
    let fill = if active {
        Color32::from_rgb(20, 52, 100)
    } else {
        SURFACE
    };
    let border_col = if active { ACCENT } else { BORDER };
    let text_col = if active { ACCENT } else { MUTED };
    if ui
        .add_enabled(
            enabled,
            egui::Button::new(RichText::new(label).size(12.0).color(text_col))
                .fill(fill)
                .stroke(Stroke::new(if active { 1.5 } else { 1.0 }, border_col))
                .corner_radius(4.0f32)
                .min_size(Vec2::new(0.0, 26.0)),
        )
        .on_hover_text(value)
        .clicked()
    {
        *field = value.to_owned();
    }
}

/// Combo-box of preset values paired with an editable text field.
/// Selecting a preset fills the text box; the user can also type freely for
/// fully custom values. The combo shows "— preset —" when no preset matches.
fn combo_and_text(
    ui: &mut Ui,
    combo_id: &str,
    presets: &[(&str, &str)],
    field: &mut String,
    hint: &str,
    enabled: bool,
) {
    let selected_label = presets
        .iter()
        .find(|(_, v)| *v == field.as_str())
        .map(|(l, _)| *l)
        .unwrap_or("— preset —");
    ui.horizontal(|ui| {
        ui.add_enabled_ui(enabled, |ui| {
            egui::ComboBox::from_id_salt(combo_id)
                .selected_text(selected_label)
                .width(170.0)
                .show_ui(ui, |ui| {
                    for (label, val) in presets {
                        if ui
                            .selectable_label(field.as_str() == *val, *label)
                            .clicked()
                        {
                            *field = (*val).to_owned();
                        }
                    }
                });
        });
        ui.add_space(4.0);
        ui.add_enabled(
            enabled,
            egui::TextEdit::singleline(field)
                .hint_text(hint)
                .desired_width(f32::INFINITY),
        );
    });
}

fn now_ts() -> String {
    chrono::Utc::now().format("%H:%M:%S UTC").to_string()
}

/// Map a detected `Distro` enum value to the lowercase string used by the
/// inject form's distro selector (must match the chip values in
/// `show_inject_configure`).
fn iso_distro_to_form_distro(d: forgeiso_engine::Distro) -> String {
    use forgeiso_engine::Distro;
    match d {
        Distro::Ubuntu => "ubuntu".into(),
        Distro::Mint => "mint".into(),
        Distro::Fedora => "fedora".into(),
        Distro::Arch => "arch".into(),
    }
}

/// Map a preset's static `distro` string (from `IsoPreset.distro`) to the
/// form distro value.  RHEL-family presets use "fedora" because the engine
/// generates Kickstart YAML for that path; Debian/Kali/openSUSE fall back to
/// "ubuntu" (cloud-init / preseed).
fn preset_distro_to_form_distro(preset_distro: &str) -> String {
    match preset_distro {
        "mint" => "mint".into(),
        "fedora" | "rhel-family" => "fedora".into(),
        "arch" => "arch".into(),
        // debian, kali, opensuse, ubuntu, pop-os all use the ubuntu/cloud-init path
        _ => "ubuntu".into(),
    }
}

// Maximum number of log entries retained in memory.  Once this limit is
// reached, the oldest 20 % of entries are evicted to keep memory bounded.
const MAX_LOG_ENTRIES: usize = 10_000;

// ── App state ─────────────────────────────────────────────────────────────────

/// Linear 4-step pipeline replacing the old Tab + nested InjectStep system.
#[derive(Clone, Copy, PartialEq, Eq, Default)]
enum AppStep {
    #[default]
    Source, // 1 — Pick the source ISO
    Configure, // 2 — Set up autoinstall options
    Build,     // 3 — Build My ISO (run inject)
    Check,     // 4 — Verify & inspect the output
}

pub struct ForgeApp {
    rt: tokio::runtime::Runtime,
    engine: Arc<ForgeIsoEngine>,
    tx: mpsc::Sender<WorkerMsg>,
    rx: mpsc::Receiver<WorkerMsg>,
    // Navigation — single linear step, no nested state machines
    app_step: AppStep,
    doctor_open: bool, // floating doctor window
    // Job
    job_running: bool,
    job_phase: String,
    job_pct: Option<f32>,
    current_task: Option<tokio::task::JoinHandle<()>>,
    detect_task: Option<tokio::task::JoinHandle<()>>,
    sha256_task: Option<tokio::task::JoinHandle<()>>,
    // Forms
    inject: InjectState,
    verify: VerifyState,
    // Results
    inject_result: Option<BuildResult>,
    inject_sha256: Option<String>,
    verify_result: Option<VerifyResult>,
    iso9660_result: Option<Iso9660Compliance>,
    // Done flags
    inject_done: bool,
    verify_done: bool,
    // Log
    log_entries: Vec<LogEntry>,
    log_open: bool,
    download_idx: std::collections::HashMap<String, usize>,
    log_errors_only: bool,
    // Status
    status: Option<StatusMsg>,
    status_since: Option<std::time::Instant>,
    // Doctor
    doctor_result: Option<DoctorReport>,
    /// Set to `true` once on startup; consumed on the first frame to kick off
    /// an initial doctor check without blocking the constructor.
    needs_initial_doctor: bool,
}

impl ForgeApp {
    pub fn new(cc: &eframe::CreationContext<'_>, rt: tokio::runtime::Runtime) -> Self {
        let mut visuals = egui::Visuals::dark();
        visuals.window_fill = BG;
        visuals.panel_fill = BG;
        visuals.widgets.noninteractive.bg_fill = SURFACE;
        visuals.widgets.noninteractive.fg_stroke = Stroke::new(1.0, TEXT);
        visuals.widgets.inactive.bg_fill = Color32::from_rgb(27, 32, 39);
        visuals.widgets.inactive.fg_stroke = Stroke::new(1.0, TEXT);
        visuals.widgets.hovered.bg_fill = Color32::from_rgb(33, 38, 45);
        visuals.widgets.active.bg_fill = ACCENT;
        visuals.selection.bg_fill = Color32::from_rgba_premultiplied(47, 129, 247, 70);
        visuals.override_text_color = Some(TEXT);
        visuals.window_stroke = Stroke::new(1.0, BORDER);
        cc.egui_ctx.set_visuals(visuals);

        let mut style = (*cc.egui_ctx.style()).clone();
        use egui::{FontId, TextStyle};
        style.text_styles = [
            (TextStyle::Heading, FontId::proportional(18.0)),
            (TextStyle::Body, FontId::proportional(14.0)),
            (TextStyle::Button, FontId::proportional(13.0)),
            (TextStyle::Small, FontId::proportional(11.0)),
            (TextStyle::Monospace, FontId::monospace(13.0)),
        ]
        .into();
        cc.egui_ctx.set_style(style);

        let (tx, rx) = mpsc::channel();
        let engine = Arc::new(ForgeIsoEngine::new());

        let persisted: PersistedState = cc
            .storage
            .and_then(|s| eframe::get_value(s, STORAGE_KEY))
            .unwrap_or_default();

        {
            let mut ev_rx = engine.subscribe();
            let tx2 = tx.clone();
            rt.spawn(async move {
                while let Ok(ev) = ev_rx.recv().await {
                    use forgeiso_engine::EventLevel;
                    let is_error = matches!(ev.level, EventLevel::Error);
                    let is_warn = matches!(ev.level, EventLevel::Warn);
                    let _ = tx2.send(WorkerMsg::EngineEvent {
                        phase: format!("{:?}", ev.phase),
                        message: ev.message.clone(),
                        percent: ev.percent.map(|p| p / 100.0),
                        is_error,
                        is_warn,
                    });
                }
            });
        }

        Self {
            rt,
            engine,
            tx,
            rx,
            app_step: AppStep::Source,
            doctor_open: false,
            job_running: false,
            job_phase: String::new(),
            job_pct: None,
            current_task: None,
            detect_task: None,
            sha256_task: None,
            inject: persisted.inject,
            verify: persisted.verify,
            inject_result: None,
            inject_sha256: None,
            verify_result: None,
            iso9660_result: None,
            inject_done: false,
            verify_done: false,
            log_entries: Vec::new(),
            log_open: false,
            download_idx: std::collections::HashMap::new(),
            log_errors_only: false,
            status: None,
            status_since: None,
            doctor_result: None,
            needs_initial_doctor: true,
        }
    }

    // ── Message handling ───────────────────────────────────────────────────────

    fn drain_messages(&mut self, ctx: &egui::Context) {
        // Cap at 2 000 messages per frame to prevent UI stutter on high-volume
        // event bursts (e.g. rapid download progress ticks).
        for _ in 0..2000 {
            match self.rx.try_recv() {
                Ok(msg) => self.handle_msg(msg, ctx),
                Err(_) => break,
            }
        }
        ctx.request_repaint();
        if let Some(t) = self.status_since {
            if t.elapsed().as_secs() >= 8
                && self.status.as_ref().map(|s| !s.is_error).unwrap_or(false)
            {
                self.status = None;
                self.status_since = None;
            }
        }
    }

    fn handle_msg(&mut self, msg: WorkerMsg, ctx: &egui::Context) {
        match msg {
            WorkerMsg::EngineEvent {
                phase,
                message,
                percent,
                is_error,
                is_warn,
            } => {
                self.job_phase = phase.clone();
                self.job_pct = percent;
                let level = if is_error {
                    LogLevel::Error
                } else if is_warn {
                    LogLevel::Warn
                } else {
                    LogLevel::Info
                };
                if let Some(pct) = percent {
                    // Download progress — update the existing bar entry or create one.
                    let pct_u8 = (pct * 100.0).clamp(0.0, 100.0) as u8;
                    let bar = make_progress_bar(pct_u8);
                    let display = format!("{message}  {bar} {pct_u8}%");
                    if let Some(&idx) = self.download_idx.get(&phase) {
                        if let Some(entry) = self.log_entries.get_mut(idx) {
                            entry.message = display;
                            entry.percent = Some(pct_u8);
                        }
                    } else {
                        self.evict_log_if_full();
                        let idx = self.log_entries.len();
                        self.download_idx.insert(phase.clone(), idx);
                        self.log_entries.push(LogEntry {
                            phase,
                            message: display,
                            level,
                            timestamp: now_ts(),
                            percent: Some(pct_u8),
                        });
                    }
                } else {
                    // Regular message — clear any in-progress bar for this phase.
                    self.download_idx.remove(&phase);
                    self.evict_log_if_full();
                    self.log_entries.push(LogEntry {
                        phase,
                        message,
                        level,
                        timestamp: now_ts(),
                        percent: None,
                    });
                }
            }
            WorkerMsg::InjectOk(r) => {
                self.inject_done = true;
                self.inject_sha256 = None; // will be populated by Sha256Ready
                egui::Popup::close_all(ctx);
                // Inject complete — advance directly to Check (step 4).
                self.app_step = AppStep::Check;
                // Auto-fill build source from the inject output so the user
                // doesn't have to manually copy-paste the path.
                if let Some(path) = r.artifacts.first() {
                    let out = path.to_string_lossy().into_owned();
                    self.verify.source = out;
                    // Spawn background SHA-256 computation of the output ISO.
                    // Uses spawn_blocking because reading 4–6 GB is a blocking op.
                    let iso_path = path.clone();
                    let tx2 = self.tx.clone();
                    // Store the handle so cancel_job() can abort the read
                    // before a stale hash overwrites the next run's result.
                    self.sha256_task = Some(self.rt.spawn(async move {
                        let digest = tokio::task::spawn_blocking(move || {
                            use sha2::{Digest, Sha256};
                            use std::io::Read;
                            let mut file = std::fs::File::open(&iso_path).ok()?;
                            let mut hasher = Sha256::new();
                            let mut buf = vec![0u8; 1 << 20]; // 1 MiB chunks
                            loop {
                                let n = file.read(&mut buf).ok()?;
                                if n == 0 {
                                    break;
                                }
                                hasher.update(&buf[..n]);
                            }
                            Some(format!("{:x}", hasher.finalize()))
                        })
                        .await
                        .ok()
                        .flatten();
                        match digest {
                            Some(hash) => {
                                let _ = tx2.send(WorkerMsg::Sha256Ready(hash));
                            }
                            None => {
                                // File read failed (deleted, permission error, etc.)
                                // Send an empty string so the UI shows "unavailable"
                                // rather than spinning forever.
                                let _ = tx2.send(WorkerMsg::Sha256Ready(String::new()));
                            }
                        }
                    }));
                }
                self.inject_result = Some(*r);
                self.finish_job();
                self.set_status(StatusMsg::ok("Inject complete"));
            }
            WorkerMsg::Sha256Ready(hash) => {
                self.inject_sha256 = Some(hash);
            }
            WorkerMsg::VerifyOk(r) => {
                let matched = r.matched;
                self.verify_result = Some(*r);
                self.verify_done = true;
                self.finish_job();
                self.set_status(if matched {
                    StatusMsg::ok("Checksum matched")
                } else {
                    StatusMsg::err("Checksum mismatch")
                });
            }
            WorkerMsg::Iso9660Ok(r) => {
                let ok = r.compliant;
                self.iso9660_result = Some(*r);
                self.finish_job();
                self.set_status(if ok {
                    StatusMsg::ok("ISO-9660 compliant")
                } else {
                    StatusMsg::err("ISO-9660 non-compliant")
                });
            }
            WorkerMsg::DiffOk => {
                // Diff removed from pipeline — no-op.
                self.finish_job();
            }
            WorkerMsg::IsoDetected(m) => {
                // Guard: if a job is running (inject/build/verify/…) the user
                // may have already changed the source or started a new operation;
                // applying stale detection results would corrupt form state.
                if self.job_running {
                    return;
                }
                // Background detect triggered after a file is picked for the
                // inject source.  Quietly fill in distro and volume label;
                // never overwrite fields the user has already set manually.
                if let Some(distro) = m.distro {
                    if self.inject.distro == "ubuntu" || self.inject.distro.is_empty() {
                        self.inject.distro = iso_distro_to_form_distro(distro);
                    }
                }
                if let Some(vol) = &m.volume_id {
                    if self.inject.output_label.is_empty() {
                        self.inject.output_label = vol.chars().take(32).collect();
                    }
                }
                // Not a blocking job — do not clear job_running here.
            }
            WorkerMsg::DoctorOk(r) => {
                self.doctor_result = Some(*r);
                self.finish_job();
                self.set_status(StatusMsg::ok("Doctor check complete"));
            }
            WorkerMsg::FilePicked { target, path } => {
                // Guard: don't overwrite source/output fields mid-job.
                // The zenity dialog was open before the job started; the user
                // may have been slow to pick.  Silently discard if running.
                if self.job_running {
                    return;
                }
                match target {
                    PickTarget::InjectSource => {
                        // Abort any stale detection task so its result cannot
                        // overwrite the distro/label for the newly chosen ISO.
                        if let Some(h) = self.detect_task.take() {
                            h.abort();
                        }
                        // Changing source invalidates any previous inject result.
                        if self.inject.source != path {
                            self.inject_done = false;
                            self.inject_result = None;
                            self.inject_sha256 = None;
                        }
                        self.inject.source = path.clone();
                        // Trigger a background ISO inspect to auto-detect distro,
                        // volume label, and architecture — result arrives as
                        // WorkerMsg::IsoDetected and quietly fills form fields.
                        self.spawn_detect_iso(path);
                    }
                    PickTarget::InjectOutputDir => self.inject.output_dir = path,
                    PickTarget::InjectWallpaper => self.inject.wallpaper_path = path,
                    PickTarget::VerifySource => {
                        // Abort any in-flight verify so its result doesn't
                        // overwrite the check for the newly chosen file.
                        if let Some(h) = self.current_task.take() {
                            h.abort();
                        }
                        // Changing source invalidates previous verify result.
                        if self.verify.source != path {
                            self.verify_result = None;
                            self.verify_done = false;
                            self.iso9660_result = None;
                        }
                        self.verify.source = path;
                    }
                }
            }
            WorkerMsg::OpError(e) => {
                self.finish_job();
                self.log_entries.push(LogEntry {
                    phase: "Error".into(),
                    message: e.clone(),
                    level: LogLevel::Error,
                    timestamp: now_ts(),
                    percent: None,
                });
                self.set_status(StatusMsg::err(e));
            }
            WorkerMsg::Done => {
                self.finish_job();
            }
        }
    }

    // ── Engine spawn helpers ───────────────────────────────────────────────────

    /// Mark a job as finished: clear the running flag, phase label, and pct.
    /// Call this from every terminal WorkerMsg handler instead of setting
    /// `job_running = false` directly so the header never shows a stale phase.
    fn finish_job(&mut self) {
        self.job_running = false;
        self.job_phase.clear();
        self.job_pct = None;
    }

    fn start_job(&mut self, phase: &str) {
        self.job_running = true;
        self.job_phase = phase.into();
        self.job_pct = None;
        self.status = None;
        self.status_since = None;
        // Reset the phase→log-index map so new progress events for any phase
        // (e.g. "Download") create fresh entries instead of in-place-updating
        // entries from a prior operation that are still visible in the log.
        self.download_idx.clear();
        // Auto-open log panel when a job starts so progress is visible.
        self.log_open = true;
    }

    fn set_status(&mut self, msg: StatusMsg) {
        self.status_since = Some(std::time::Instant::now());
        self.status = Some(msg);
    }

    /// Evict the oldest 20 % of log entries when the cap is reached.
    /// Also clears `download_idx` since stored indices become invalid after eviction.
    fn evict_log_if_full(&mut self) {
        if self.log_entries.len() >= MAX_LOG_ENTRIES {
            let evict = MAX_LOG_ENTRIES / 5;
            self.log_entries.drain(0..evict);
            // Indices stored in download_idx now point to wrong entries.
            self.download_idx.clear();
        }
    }

    fn cancel_job(&mut self) {
        if let Some(handle) = self.current_task.take() {
            handle.abort();
        }
        // Also abort any in-flight background ISO detection so a stale
        // IsoDetected message cannot corrupt the next operation's form state.
        if let Some(handle) = self.detect_task.take() {
            handle.abort();
        }
        // Abort the background SHA-256 hash task — it reads the full ISO (4–6 GB)
        // and can arrive long after the user cancelled, overwriting inject_sha256.
        if let Some(handle) = self.sha256_task.take() {
            handle.abort();
        }
        // Drain any in-flight messages from the aborted task so they don't
        // corrupt the state of the next operation (e.g. setting job_running=false
        // while a new task is already running, or overwriting fresh results).
        while self.rx.try_recv().is_ok() {}
        self.job_running = false;
        self.job_phase.clear();
        self.job_pct = None;
        self.download_idx.clear();
        // Reset all per-operation done flags so step bars don't show stale ✓
        // after a cancelled run.  Results are left intact so the user can still
        // read the last successful output; they are cleared when a new job starts.
        self.inject_done = false;
        self.verify_done = false;
        self.set_status(StatusMsg::ok("Cancelled"));
    }

    fn spawn_inject(&mut self) {
        if self.job_running {
            return;
        }
        if self.inject.source.trim().is_empty() {
            self.set_status(StatusMsg::err(
                "Source ISO is required — pick a file or paste a URL",
            ));
            return;
        }
        if self.inject.output_dir.trim().is_empty() {
            self.set_status(StatusMsg::err("Output directory is required"));
            return;
        }
        if !std::path::Path::new(self.inject.output_dir.trim()).exists() {
            self.set_status(StatusMsg::err(
                "Output directory does not exist — create it first or choose a different path",
            ));
            return;
        }
        if !self.inject.password.is_empty()
            && !self.inject.password_confirm.is_empty()
            && self.inject.password != self.inject.password_confirm
        {
            self.set_status(StatusMsg::err(
                "Passwords do not match — please confirm your password",
            ));
            return;
        }
        // Silently append .iso if the user dropped the extension.
        if !self.inject.out_name.trim().is_empty()
            && !self.inject.out_name.to_lowercase().ends_with(".iso")
        {
            self.inject.out_name.push_str(".iso");
        }
        // ISO 9660 volume label is max 32 characters (not bytes — use chars()).
        if !self.inject.output_label.is_empty() && self.inject.output_label.chars().count() > 32 {
            self.set_status(StatusMsg::err(
                "Volume label exceeds 32 characters — ISO-9660 maximum",
            ));
            return;
        }
        // Pre-flight: validate config synchronously before starting the async
        // task so validation errors surface immediately without a spinner flash.
        let preflight = build_inject_config(&self.inject);
        if let Err(e) = preflight.validate() {
            self.set_status(StatusMsg::err(format!("Config error: {e}")));
            return;
        }
        self.inject_done = false;
        self.inject_result = None;
        self.inject_sha256 = None;
        // Abort any in-flight background ISO detection — a stale IsoDetected
        // message must not overwrite distro/label mid-build.
        if let Some(h) = self.detect_task.take() {
            h.abort();
        }
        // Abort any previous SHA-256 hash task so its stale result cannot
        // arrive and overwrite the hash for this new injection run.
        if let Some(h) = self.sha256_task.take() {
            h.abort();
        }
        self.start_job("Injecting…");
        let engine = Arc::clone(&self.engine);
        let tx = self.tx.clone();
        let inject = self.inject.clone();
        let out = PathBuf::from(&inject.output_dir);
        self.current_task = Some(self.rt.spawn(async move {
            let cfg = build_inject_config(&inject);
            match engine.inject_autoinstall(&cfg, &out).await {
                Ok(r) => {
                    let _ = tx.send(WorkerMsg::InjectOk(Box::new(r)));
                }
                Err(e) => {
                    let _ = tx.send(WorkerMsg::OpError(e.to_string()));
                }
            }
        }));
    }

    /// Background ISO inspection for distro/label auto-detection after a file
    /// is picked.  Does NOT set `job_running` — it runs concurrently with
    /// normal UI without blocking the Run button.  The handle is stored in
    /// `detect_task` so `cancel_job()` can abort it before the result arrives.
    fn spawn_detect_iso(&mut self, source: String) {
        let engine = Arc::clone(&self.engine);
        let tx = self.tx.clone();
        let handle = self.rt.spawn(async move {
            if let Ok(m) = engine.inspect_source(&source, None).await {
                let _ = tx.send(WorkerMsg::IsoDetected(Box::new(m)));
            }
            // Errors are silently ignored — detection is best-effort.
        });
        self.detect_task = Some(handle);
    }

    fn spawn_verify(&mut self) {
        if self.job_running {
            return;
        }
        // Abort any previous verify task — double-click or rapid re-run must
        // not create two concurrent writers to verify_result.
        if let Some(h) = self.current_task.take() {
            h.abort();
        }
        if self.verify.source.trim().is_empty() {
            self.set_status(StatusMsg::err(
                "Source ISO is required — pick a file or paste a URL",
            ));
            return;
        }
        // Normalize whitespace in URL/path before engine sees it.
        self.verify.source = self.verify.source.trim().to_string();
        self.verify_done = false;
        self.verify_result = None;
        self.iso9660_result = None;
        self.start_job("Verifying checksum…");
        let engine = Arc::clone(&self.engine);
        let tx = self.tx.clone();
        let source = self.verify.source.clone();
        let sums = opt(&self.verify.sums_url);
        self.current_task = Some(self.rt.spawn(async move {
            match engine.verify(&source, sums.as_deref()).await {
                Ok(r) => {
                    let _ = tx.send(WorkerMsg::VerifyOk(Box::new(r)));
                }
                Err(e) => {
                    let _ = tx.send(WorkerMsg::OpError(e.to_string()));
                }
            }
        }));
    }

    fn spawn_iso9660(&mut self) {
        if self.job_running {
            return;
        }
        // Abort any previous ISO-9660 task — rapid double-click must not race.
        if let Some(h) = self.current_task.take() {
            h.abort();
        }
        self.iso9660_result = None;
        self.start_job("Validating ISO-9660…");
        let engine = Arc::clone(&self.engine);
        let tx = self.tx.clone();
        let source = self.verify.source.clone();
        self.current_task = Some(self.rt.spawn(async move {
            match engine.validate_iso9660(&source).await {
                Ok(r) => {
                    let _ = tx.send(WorkerMsg::Iso9660Ok(Box::new(r)));
                }
                Err(e) => {
                    let _ = tx.send(WorkerMsg::OpError(e.to_string()));
                }
            }
        }));
    }

    // spawn_diff removed — Diff tab is no longer part of the pipeline.

    fn spawn_doctor(&mut self) {
        if self.job_running {
            return;
        }
        // Clear stale result so the UI shows the spinner while the new run is
        // in flight rather than keeping the previous check's output visible.
        self.doctor_result = None;
        self.start_job("Checking dependencies…");
        let engine = Arc::clone(&self.engine);
        let tx = self.tx.clone();
        self.current_task = Some(self.rt.spawn(async move {
            let r = engine.doctor().await;
            let _ = tx.send(WorkerMsg::DoctorOk(Box::new(r)));
        }));
    }

    // ── Rendering ─────────────────────────────────────────────────────────────

    /// Top header bar: logo + status + cancel button.
    fn render_header(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::top("header")
            .frame(
                Frame::new()
                    .fill(PANEL_BG)
                    .stroke(Stroke::NONE)
                    .inner_margin(egui::Margin::symmetric(20, 12)),
            )
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    // Logo
                    ui.label(
                        RichText::new("ForgeISO")
                            .size(18.0)
                            .strong()
                            .color(Color32::WHITE),
                    );
                    ui.add_space(6.0);
                    ui.label(
                        RichText::new("Build hands-free Linux installers — boot and walk away")
                            .size(13.0)
                            .color(MUTED),
                    );

                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if self.job_running {
                            // Cancel — rightmost item in right-to-left layout
                            if ui
                                .add(
                                    egui::Button::new(
                                        RichText::new("✕  Cancel").size(13.0).color(Color32::WHITE),
                                    )
                                    .fill(Color32::from_rgb(90, 25, 25))
                                    .stroke(Stroke::new(1.0, RED))
                                    .corner_radius(6.0f32)
                                    .min_size(Vec2::new(84.0, 30.0)),
                                )
                                .clicked()
                            {
                                self.cancel_job();
                            }
                            ui.add_space(12.0);
                            // Phase label
                            ui.label(RichText::new(&self.job_phase).size(13.0).color(MUTED));
                            ui.add_space(10.0);
                            // Progress bar (deterministic) or spinner (indeterminate)
                            if let Some(pct) = self.job_pct {
                                ui.add(
                                    egui::ProgressBar::new(pct)
                                        .desired_width(180.0)
                                        .animate(true)
                                        .show_percentage(),
                                );
                            } else {
                                ui.spinner();
                            }
                        } else if let Some(s) = self.status.as_ref() {
                            let (col, icon) = if s.is_error {
                                (RED, "✗")
                            } else {
                                (GREEN, "✓")
                            };
                            ui.label(
                                RichText::new(format!("{icon}  {}", s.text))
                                    .size(13.0)
                                    .color(col),
                            );
                        }
                    });
                });
            });
    }

    /// 4-step pipeline bar: Source → Configure → Build → Check.
    fn render_step_bar(&mut self, ctx: &egui::Context) {
        // Step definitions: (variant, badge_num, label, sublabel, done_flag)
        let steps: &[(AppStep, &str, &str, &str, bool)] = &[
            (
                AppStep::Source,
                "1",
                "Source",
                "Pick your ISO",
                !self.inject.source.trim().is_empty(),
            ),
            (
                AppStep::Configure,
                "2",
                "Configure",
                "Autoinstall options",
                self.inject_done,
            ),
            (
                AppStep::Build,
                "3",
                "Build",
                "Build My ISO",
                self.inject_done,
            ),
            (
                AppStep::Check,
                "4",
                "Check",
                "Verify & inspect",
                self.verify_done,
            ),
        ];

        egui::TopBottomPanel::top("step_bar")
            .frame(
                Frame::new()
                    .fill(PANEL_BG)
                    .stroke(Stroke::NONE)
                    .inner_margin(egui::Margin::symmetric(20, 10)),
            )
            .show(ctx, |ui| {
                // Single clean separator between header and step bar
                let sep_w = ui.available_width();
                let (sep_rect, _) =
                    ui.allocate_exact_size(Vec2::new(sep_w, 1.0), egui::Sense::hover());
                ui.painter().rect_filled(sep_rect, 0.0, BORDER);
                ui.add_space(8.0);
                ui.horizontal(|ui| {
                    ui.add_space(4.0);

                    for (i, (step, num, label, sublabel, done)) in steps.iter().enumerate() {
                        let current = self.app_step == *step;
                        let (badge_fill, badge_text_col) = if *done {
                            (GREEN, PANEL_BG)
                        } else if current {
                            (ACCENT, Color32::WHITE)
                        } else {
                            (Color32::from_rgb(28, 33, 40), MUTED)
                        };
                        let badge_num = if *done { "✓" } else { *num };
                        let label_col = if current {
                            TEXT
                        } else if *done {
                            GREEN
                        } else {
                            MUTED
                        };

                        let clicked = ui
                            .vertical(|ui| {
                                ui.horizontal(|ui| {
                                    Frame::new()
                                        .fill(badge_fill)
                                        .stroke(Stroke::new(
                                            1.5,
                                            if current || *done { ACCENT } else { BORDER },
                                        ))
                                        .corner_radius(14.0f32)
                                        .show(ui, |ui| {
                                            ui.set_min_size(Vec2::new(26.0, 26.0));
                                            ui.centered_and_justified(|ui| {
                                                ui.label(
                                                    RichText::new(badge_num)
                                                        .size(12.0)
                                                        .strong()
                                                        .color(badge_text_col),
                                                );
                                            });
                                        });
                                    ui.add_space(6.0);
                                    ui.vertical(|ui| {
                                        let rt = RichText::new(*label).size(13.0).color(label_col);
                                        ui.label(if current { rt.strong() } else { rt });
                                        ui.label(RichText::new(*sublabel).size(10.0).color(
                                            if current {
                                                Color32::from_rgb(80, 100, 130)
                                            } else {
                                                Color32::from_rgb(48, 54, 61)
                                            },
                                        ));
                                    });
                                });
                                // Invisible click target over the whole cell
                                ui.interact(
                                    ui.min_rect(),
                                    ui.id().with(("step_click", i)),
                                    egui::Sense::click(),
                                )
                                .clicked()
                            })
                            .inner;

                        if clicked && !self.job_running {
                            // Allow backward navigation freely; forward navigation only to
                            // completed steps — prevents skipping incomplete configure/build steps.
                            let current_idx = steps
                                .iter()
                                .position(|(s, ..)| s == &self.app_step)
                                .unwrap_or(0);
                            if i <= current_idx || *done {
                                self.app_step = *step;
                            }
                        }

                        // Connector line between steps — 2px thick, centered
                        if i + 1 < steps.len() {
                            ui.add_space(6.0);
                            let connector_col = if *done {
                                ACCENT
                            } else {
                                Color32::from_rgb(38, 44, 52)
                            };
                            let (rect, _) =
                                ui.allocate_exact_size(Vec2::new(48.0, 2.0), egui::Sense::hover());
                            // Draw with rounded caps for a polished look
                            ui.painter().rect_filled(rect, 1.0, connector_col);
                            ui.add_space(6.0);
                        }
                    }

                    // Log toggle on the right
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ui.add_space(4.0);
                        let error_count = self
                            .log_entries
                            .iter()
                            .filter(|e| e.level == LogLevel::Error)
                            .count();
                        let log_label = if error_count > 0 {
                            format!(
                                "⚠  {} error{}{}",
                                error_count,
                                if error_count == 1 { "" } else { "s" },
                                if self.log_errors_only {
                                    "  [filtered]"
                                } else {
                                    ""
                                },
                            )
                        } else if self.log_entries.is_empty() {
                            if self.log_errors_only {
                                "Log  [errors only]".to_string()
                            } else {
                                "Log".to_string()
                            }
                        } else if self.log_errors_only {
                            format!("Log  [errors only]  {}", self.log_entries.len())
                        } else {
                            format!("Log  {}", self.log_entries.len())
                        };
                        let log_col = if error_count > 0 {
                            RED
                        } else if self.log_errors_only {
                            AMBER
                        } else {
                            MUTED
                        };
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new(log_label).size(12.0).color(log_col),
                                )
                                .fill(Color32::TRANSPARENT)
                                .stroke(Stroke::new(0.0, BORDER)),
                            )
                            .clicked()
                        {
                            self.log_open = !self.log_open;
                        }

                        // Doctor button
                        ui.add_space(8.0);
                        if ui
                            .add(
                                egui::Button::new(RichText::new("🩺").size(16.0))
                                    .fill(Color32::TRANSPARENT)
                                    .stroke(Stroke::new(0.0, BORDER)),
                            )
                            .on_hover_text("Tool check (Doctor)")
                            .clicked()
                        {
                            self.doctor_open = !self.doctor_open;
                        }
                    });
                });
                // Single clean separator between step bar and main content
                ui.add_space(8.0);
                let sep_w = ui.available_width();
                let (sep_rect, _) =
                    ui.allocate_exact_size(Vec2::new(sep_w, 1.0), egui::Sense::hover());
                ui.painter().rect_filled(sep_rect, 0.0, BORDER);
            });
    }

    /// Collapsible log strip at the bottom.
    /// `log_open` controls visibility via `show_animated` so the Log toggle
    /// button in the tab bar actually collapses/expands the panel.
    fn render_log(&mut self, ctx: &egui::Context) {
        egui::TopBottomPanel::bottom("log_panel")
            .resizable(true)
            .min_height(150.0)
            .default_height(220.0)
            .frame(
                Frame::new()
                    .fill(PANEL_BG)
                    .stroke(Stroke::NONE)
                    .inner_margin(egui::Margin::symmetric(12, 8)),
            )
            .show_animated(ctx, self.log_open, |ui| {
                // Top separator between main content and log panel
                let sep_w = ui.available_width();
                let (sep_rect, _) =
                    ui.allocate_exact_size(Vec2::new(sep_w, 1.0), egui::Sense::hover());
                ui.painter().rect_filled(sep_rect, 0.0, BORDER);
                ui.add_space(6.0);
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Log").size(13.0).strong().color(MUTED));
                    ui.add_space(12.0);
                    ui.checkbox(
                        &mut self.log_errors_only,
                        RichText::new("Errors only").size(13.0).color(MUTED),
                    );
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            .add(
                                egui::Button::new(RichText::new("Clear").size(13.0).color(MUTED))
                                    .fill(Color32::TRANSPARENT)
                                    .stroke(Stroke::new(0.0, BORDER)),
                            )
                            .clicked()
                        {
                            self.log_entries.clear();
                            self.download_idx.clear();
                        }
                    });
                });
                ui.separator();
                egui::ScrollArea::vertical()
                    .stick_to_bottom(true)
                    .show(ui, |ui| {
                        for entry in &self.log_entries {
                            if self.log_errors_only && entry.level != LogLevel::Error {
                                continue;
                            }
                            let col = if entry.percent.is_some() {
                                LOG_PROGRESS
                            } else {
                                match entry.level {
                                    LogLevel::Error => RED,
                                    LogLevel::Warn => AMBER,
                                    LogLevel::Info => MUTED,
                                }
                            };
                            ui.horizontal(|ui| {
                                ui.label(
                                    RichText::new(&entry.timestamp)
                                        .size(13.0)
                                        .monospace()
                                        .color(LOG_TIMESTAMP),
                                );
                                ui.add_space(4.0);
                                ui.label(
                                    RichText::new(format!("[{}]", entry.phase))
                                        .size(13.0)
                                        .monospace()
                                        .color(ACCENT),
                                );
                                ui.add_space(4.0);
                                ui.label(
                                    RichText::new(&entry.message)
                                        .size(13.0)
                                        .monospace()
                                        .color(col),
                                );
                            });
                        }
                    });
            });
    }

    // ── Step content ───────────────────────────────────────────────────────────

    /// Step 1 (Source): Source ISO selection — distro preset cards + URL/file field.
    /// Advances to AppStep::Configure when the user picks a source.
    fn show_step_source(&mut self, ui: &mut Ui) {
        let running = self.job_running;
        egui::ScrollArea::vertical()
            .id_salt("download_scroll")
            .show(ui, |ui| {
                ui.label(
                    RichText::new("1 — Choose Your Linux ISO")
                        .size(20.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(4.0);
                ui.label(
                    RichText::new(
                        "Pick a distro below to auto-fill the download URL, \
                         or paste your own ISO path / URL. Then click Continue.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(16.0);
                self.show_distro_cards(ui, running);
                ui.add_space(16.0);
                let can_continue = !self.inject.source.trim().is_empty() && !running;
                if action_btn(ui, "Continue: Configure Installer  →", can_continue) {
                    self.inject.source = self.inject.source.trim().to_string();
                    self.app_step = AppStep::Configure;
                }
                ui.add_space(16.0);
            });
    }

    /// Shared distro-card grid (called from show_step_download and show_step_inject).
    fn show_distro_cards(&mut self, ui: &mut Ui, running: bool) {
        section(ui, "Choose Your Linux");
        let top_picks: &[(&str, &str, &str, &str)] = &[
            (
                "ubuntu-server-lts",
                "🟠",
                "Ubuntu Server",
                "LTS · unattended",
            ),
            (
                "ubuntu-desktop-lts",
                "🟠",
                "Ubuntu Desktop",
                "LTS · full GUI",
            ),
            (
                "linux-mint-cinnamon",
                "🟢",
                "Linux Mint",
                "Beginner-friendly",
            ),
            ("fedora-workstation", "🔵", "Fedora", "Cutting-edge"),
            ("rocky-linux", "🪨", "Rocky Linux", "RHEL-compatible"),
            ("arch-linux", "🏹", "Arch Linux", "Rolling release"),
            ("debian-netinst", "🌀", "Debian", "Rock-solid"),
            ("pop-os-24-intel", "🚀", "Pop!_OS", "Gaming & science"),
        ];
        let gap = 10.0;
        let card_w = ((ui.available_width() - gap * 3.0).max(0.0) / 4.0).max(110.0);
        for row in top_picks.chunks(4) {
            ui.horizontal(|ui| {
                for (preset_id, emoji, name, desc) in row {
                    let selected = self.inject.source_preset == *preset_id
                        && !self.inject.source.trim().is_empty();
                    let (fill, border_col) = if selected {
                        (Color32::from_rgb(18, 48, 95), ACCENT)
                    } else {
                        (SURFACE, BORDER)
                    };
                    let resp = Frame::new()
                        .fill(fill)
                        .stroke(Stroke::new(if selected { 2.0 } else { 1.0 }, border_col))
                        .corner_radius(10.0f32)
                        .inner_margin(12.0f32)
                        .show(ui, |ui| {
                            ui.set_width(card_w);
                            ui.vertical_centered(|ui| {
                                ui.label(RichText::new(*emoji).size(28.0));
                                ui.add_space(5.0);
                                ui.label(
                                    RichText::new(*name).size(13.0).strong().color(if selected {
                                        ACCENT
                                    } else {
                                        TEXT
                                    }),
                                );
                                ui.add_space(1.0);
                                ui.label(RichText::new(*desc).size(11.0).color(MUTED));
                                if selected {
                                    ui.add_space(3.0);
                                    ui.label(RichText::new("✓").size(12.0).color(ACCENT));
                                }
                            });
                        });
                    if !running && resp.response.interact(egui::Sense::click()).clicked() {
                        if let Some(p) = find_preset_by_str(preset_id) {
                            self.inject.source_preset = p.id.as_str().to_string();
                            self.inject.distro = preset_distro_to_form_distro(p.distro);
                            // Always reset prior build state when switching presets —
                            // the result belongs to the old preset, not the new one.
                            self.inject_done = false;
                            self.inject_result = None;
                            self.inject_sha256 = None;
                            if p.strategy == AcquisitionStrategy::DirectUrl {
                                if let Ok(Some(url)) = resolve_url(p) {
                                    self.inject.source = url;
                                }
                            }
                            // Auto-advance to Configure so the user immediately
                            // sees the pre-filled settings for the chosen distro.
                            self.app_step = AppStep::Configure;
                        }
                    }
                    ui.add_space(gap);
                }
            });
            ui.add_space(gap);
        }

        // Source URL / file picker
        section(ui, "Source ISO");
        let has_source = !self.inject.source.trim().is_empty();
        let text_w = (ui.available_width() - 104.0 - if has_source { 46.0 } else { 0.0 }).max(50.0);
        ui.horizontal(|ui| {
            ui.add_enabled(
                !running,
                egui::TextEdit::singleline(&mut self.inject.source)
                    .hint_text("/path/to/ubuntu.iso  or  https://releases.ubuntu.com/…")
                    .desired_width(text_w)
                    .min_size(Vec2::new(0.0, 38.0)),
            );
            if browse_btn(ui, !running) {
                worker::pick_iso(PickTarget::InjectSource, self.tx.clone());
            }
            if has_source
                && ui
                    .add_enabled(
                        !running,
                        egui::Button::new(RichText::new("✕").size(14.0).color(MUTED))
                            .fill(Color32::TRANSPARENT)
                            .stroke(Stroke::new(0.0, BORDER))
                            .min_size(Vec2::new(34.0, 38.0)),
                    )
                    .on_hover_text("Clear source")
                    .clicked()
            {
                self.inject.source.clear();
                self.inject.source_preset.clear();
                // Also clear in-memory secrets — they are not persisted, but
                // clearing on reset avoids stale passwords carrying into a new
                // session without the user noticing.
                self.inject.password.clear();
                self.inject.password_confirm.clear();
                self.inject.encrypt_passphrase.clear();
                self.inject_done = false;
                self.inject_result = None;
                self.inject_sha256 = None;
            }
        });
        if has_source {
            ui.add_space(4.0);
            // Show distro if auto-detected (populated by IsoDetected background task)
            if !self.inject.distro.is_empty() && self.inject.distro != "ubuntu" {
                let label = match self.inject.distro.as_str() {
                    "fedora" => "Fedora / RHEL",
                    "arch" => "Arch Linux",
                    "mint" => "Linux Mint",
                    other => other,
                };
                ui.label(
                    RichText::new(format!("Detected: {label}"))
                        .size(12.0)
                        .color(GREEN),
                );
            }
        }
        if self.inject.source.trim().is_empty() {
            ui.add_space(4.0);
            ui.label(
                RichText::new("Source ISO is required to create your custom installer.")
                    .size(13.0)
                    .color(AMBER),
            );
        }
    }

    /// Step 2 (Inject): Full inject settings form + output settings + "Create ISO" button.
    fn show_step_configure(&mut self, ui: &mut Ui) {
        let running = self.job_running;
        egui::ScrollArea::vertical()
            .id_salt("inject_cfg_scroll")
            .show(ui, |ui| {
                ui.label(
                    RichText::new("Build Your Custom Linux Installer")
                        .size(20.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(4.0);
                ui.label(
                    RichText::new(
                        "Pick a distro, fill in your settings, and get a ready-to-boot ISO \
                         that installs automatically — no keyboard needed at setup time.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(16.0);

                // ── Source summary (compact — user picked this in step 1) ─────────
                section(ui, "Source ISO");
                ui.horizontal(|ui| {
                    let src = self.inject.source.trim().to_string();
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.source)
                            .hint_text("← Go back to step 1 to pick an ISO")
                            .desired_width((ui.available_width() - 104.0).max(50.0))
                            .min_size(egui::Vec2::new(0.0, 38.0)),
                    );
                    if browse_btn(ui, !running) {
                        worker::pick_iso(PickTarget::InjectSource, self.tx.clone());
                    }
                    if !src.is_empty()
                        && ui
                            .add_enabled(
                                !running,
                                egui::Button::new(RichText::new("✕").size(14.0).color(MUTED))
                                    .fill(Color32::TRANSPARENT)
                                    .stroke(Stroke::new(0.0, BORDER))
                                    .min_size(egui::Vec2::new(34.0, 38.0)),
                            )
                            .on_hover_text("Clear — go back to step 1")
                            .clicked()
                    {
                        self.inject.source.clear();
                        self.inject.source_preset.clear();
                        self.inject_done = false;
                        self.inject_result = None;
                        self.inject_sha256 = None;
                        self.app_step = AppStep::Source;
                    }
                });
                ui.add_space(6.0);
                let distro_before = self.inject.distro.clone();
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Installer format:").size(12.0).color(MUTED));
                    ui.add_space(6.0);
                    ui.add_enabled_ui(!running, |ui| {
                        egui::ComboBox::from_id_salt("distro_inline_combo")
                            .selected_text(match self.inject.distro.as_str() {
                                "fedora" => "Fedora  (Kickstart)",
                                "arch" => "Arch Linux  (archinstall)",
                                "mint" => "Linux Mint  (preseed)",
                                _ => "Ubuntu / Debian  (cloud-init)",
                            })
                            .width(240.0)
                            .show_ui(ui, |ui| {
                                ui.selectable_value(
                                    &mut self.inject.distro,
                                    "ubuntu".into(),
                                    "Ubuntu / Debian  (cloud-init)",
                                );
                                ui.selectable_value(
                                    &mut self.inject.distro,
                                    "mint".into(),
                                    "Linux Mint  (preseed)",
                                );
                                ui.selectable_value(
                                    &mut self.inject.distro,
                                    "fedora".into(),
                                    "Fedora  (Kickstart)",
                                );
                                ui.selectable_value(
                                    &mut self.inject.distro,
                                    "arch".into(),
                                    "Arch Linux  (archinstall)",
                                );
                            });
                    });
                });
                // Installer format changed — prior build result is for the old
                // format and must not be shown as valid output for the new one.
                if self.inject.distro != distro_before {
                    self.inject_done = false;
                    self.inject_result = None;
                    self.inject_sha256 = None;
                }

                ui.add_space(20.0);

                // ── Identity ──────────────────────────────────────────────
                section(ui, "Machine Setup  (required)");
                ui.columns(2, |cols| {
                    cols[0].vertical(|ui| {
                        lbl(ui, "Hostname *");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.hostname)
                                .hint_text("my-server")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 40.0)),
                        );
                    });
                    cols[1].vertical(|ui| {
                        lbl(ui, "Username *");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.username)
                                .hint_text("admin")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 40.0)),
                        );
                    });
                });
                ui.add_space(10.0);
                let mismatch = !self.inject.password.is_empty()
                    && !self.inject.password_confirm.is_empty()
                    && self.inject.password != self.inject.password_confirm;
                ui.columns(2, |cols| {
                    cols[0].vertical(|ui| {
                        lbl(ui, "Password *");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.password)
                                .password(true)
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 40.0)),
                        );
                    });
                    cols[1].vertical(|ui| {
                        lbl(ui, "Confirm Password *");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.password_confirm)
                                .password(true)
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 40.0)),
                        );
                        if mismatch {
                            ui.label(
                                RichText::new("⚠  Passwords do not match")
                                    .size(12.0)
                                    .color(RED),
                            );
                        } else if !self.inject.password.is_empty() && self.inject.password.len() < 8
                        {
                            ui.label(
                                RichText::new("Use at least 8 characters")
                                    .size(12.0)
                                    .color(AMBER),
                            );
                        }
                    });
                });

                ui.add_space(10.0);

                // ── Locale & Region ───────────────────────────────────────
                section(ui, "Locale & Region");
                ui.columns(2, |cols| {
                    cols[0].vertical(|ui| {
                        lbl(ui, "Timezone");
                        let tz_presets: &[(&str, &str)] = &[
                            ("UTC", "UTC"),
                            ("US/Eastern", "America/New_York"),
                            ("US/Central", "America/Chicago"),
                            ("US/Mountain", "America/Denver"),
                            ("US/Pacific", "America/Los_Angeles"),
                            ("UK", "Europe/London"),
                            ("Germany", "Europe/Berlin"),
                            ("France", "Europe/Paris"),
                            ("Tokyo", "Asia/Tokyo"),
                            ("Sydney", "Australia/Sydney"),
                            ("Mumbai", "Asia/Kolkata"),
                            ("Dubai", "Asia/Dubai"),
                            ("São Paulo", "America/Sao_Paulo"),
                        ];
                        combo_and_text(
                            ui,
                            "tz_combo",
                            tz_presets,
                            &mut self.inject.timezone,
                            "UTC",
                            !running,
                        );
                    });
                    cols[1].vertical(|ui| {
                        lbl(ui, "Locale");
                        let locale_presets: &[(&str, &str)] = &[
                            ("en_US.UTF-8", "en_US.UTF-8"),
                            ("en_GB.UTF-8", "en_GB.UTF-8"),
                            ("en_AU.UTF-8", "en_AU.UTF-8"),
                            ("en_CA.UTF-8", "en_CA.UTF-8"),
                            ("de_DE.UTF-8", "de_DE.UTF-8"),
                            ("fr_FR.UTF-8", "fr_FR.UTF-8"),
                            ("es_ES.UTF-8", "es_ES.UTF-8"),
                            ("pt_BR.UTF-8", "pt_BR.UTF-8"),
                            ("ja_JP.UTF-8", "ja_JP.UTF-8"),
                            ("zh_CN.UTF-8", "zh_CN.UTF-8"),
                        ];
                        combo_and_text(
                            ui,
                            "locale_combo",
                            locale_presets,
                            &mut self.inject.locale,
                            "en_US.UTF-8",
                            !running,
                        );
                    });
                });

                ui.add_space(10.0);
                ui.columns(2, |cols| {
                    cols[0].vertical(|ui| {
                        lbl(ui, "Keyboard Layout");
                        let kb_presets: &[(&str, &str)] = &[
                            ("US (qwerty)", "us"),
                            ("UK", "gb"),
                            ("German", "de"),
                            ("French (azerty)", "fr"),
                            ("Spanish", "es"),
                            ("Italian", "it"),
                            ("Portuguese", "pt"),
                            ("Colemak", "us(colemak)"),
                            ("Dvorak", "us(dvorak)"),
                        ];
                        combo_and_text(
                            ui,
                            "kb_combo",
                            kb_presets,
                            &mut self.inject.keyboard_layout,
                            "us",
                            !running,
                        );
                    });
                    cols[1].vertical(|ui| {
                        lbl(ui, "Full Name (optional)");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.realname)
                                .hint_text("Jane Smith")
                                .desired_width(f32::INFINITY),
                        );
                    });
                });

                ui.add_space(16.0);

                // ── SSH ───────────────────────────────────────────────────
                section(ui, "Remote Access (SSH)");
                ui.horizontal(|ui| {
                    ui.add_enabled(
                        !running,
                        egui::Checkbox::new(
                            &mut self.inject.ssh_install_server,
                            "Install OpenSSH server",
                        ),
                    );
                    ui.add_space(16.0);
                    ui.add_enabled(
                        !running,
                        egui::Checkbox::new(
                            &mut self.inject.ssh_password_auth,
                            "Allow password auth",
                        ),
                    );
                });
                ui.add_space(6.0);
                lbl(ui, "Authorized Keys  (one per line)");
                ui.add_enabled(
                    !running,
                    egui::TextEdit::multiline(&mut self.inject.ssh_keys)
                        .hint_text("ssh-ed25519 AAAA…")
                        .desired_width(f32::INFINITY)
                        .desired_rows(3),
                );

                ui.add_space(16.0);

                // ── Advanced (collapsed) ───────────────────────────────────
                ui.add_space(6.0);
                Frame::new()
                    .fill(Color32::from_rgb(18, 23, 30))
                    .stroke(Stroke::new(1.0, BORDER))
                    .corner_radius(8.0f32)
                    .inner_margin(egui::Margin::symmetric(12, 8))
                    .show(ui, |ui| {
                        egui::CollapsingHeader::new(
                            RichText::new("⚙  Advanced Options")
                                .size(14.0)
                                .strong()
                                .color(TEXT),
                        )
                        .default_open(false)
                        .show(ui, |ui| {
                            ui.label(
                                RichText::new(
                                    "Network, packages, repositories, services, firewall, \
                                     boot settings, encryption, and more.",
                                )
                                .size(12.0)
                                .color(MUTED),
                            );
                            ui.add_space(12.0);
                            self.show_inject_advanced(ui, running);
                        });
                    });

                ui.add_space(24.0);

                let _pw_mismatch = mismatch;

                // ── Continue to Build My ISO ───────────────────────────────
                ui.add_space(8.0);
                rule(ui);
                let can_build = !self.inject.source.trim().is_empty() && !running;
                if action_btn(ui, "Continue: Build My ISO  →", can_build) {
                    self.app_step = AppStep::Build;
                }
                ui.add_space(16.0);
            });
    }

    // ── Step 3: Build My ISO ──────────────────────────────────────────────────

    fn show_step_build(&mut self, ui: &mut Ui) {
        let running = self.job_running;
        let mut do_inject = false;
        egui::ScrollArea::vertical()
            .id_salt("build_step_scroll")
            .show(ui, |ui| {
                ui.label(
                    RichText::new("3 — Build My ISO")
                        .size(20.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(6.0);
                ui.label(
                    RichText::new(
                        "Your settings will be embedded into the source ISO. \
                         Copy the finished file to a USB stick and boot from it — \
                         everything installs automatically.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(16.0);

                // ── Back to Configure link ─────────────────────────────────
                if !running {
                    ui.horizontal(|ui| {
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new("← Edit settings")
                                        .size(13.0)
                                        .color(ACCENT),
                                )
                                .fill(Color32::TRANSPARENT)
                                .stroke(Stroke::new(0.0, BORDER)),
                            )
                            .clicked()
                        {
                            self.app_step = AppStep::Configure;
                        }
                    });
                    ui.add_space(8.0);
                }

                // Summary card — confirm what will be baked into the ISO
                Frame::new()
                    .fill(Color32::from_rgb(16, 21, 30))
                    .stroke(Stroke::new(1.0, BORDER))
                    .corner_radius(8.0f32)
                    .inner_margin(14.0f32)
                    .show(ui, |ui| {
                        ui.label(
                            RichText::new("What will be embedded in your ISO")
                                .size(13.0)
                                .strong()
                                .color(TEXT),
                        );
                        ui.add_space(8.0);
                        let distro_display = match self.inject.distro.as_str() {
                            "fedora" => "Fedora (Kickstart)",
                            "arch"   => "Arch Linux (archinstall)",
                            "mint"   => "Linux Mint (preseed)",
                            _        => "Ubuntu / Debian (cloud-init)",
                        };
                        let tz_display = if self.inject.timezone.is_empty() { "system default" } else { &self.inject.timezone };
                        let storage_display = if self.inject.storage_layout.is_empty() || self.inject.storage_layout == "direct" {
                            "direct (default)"
                        } else {
                            &self.inject.storage_layout
                        };
                        let rows: &[(&str, &str)] = &[
                            ("Distro",    distro_display),
                            ("Source",    if self.inject.source.is_empty() { "—" } else { &self.inject.source }),
                            ("Hostname",  if self.inject.hostname.is_empty() { "— (not set)" } else { &self.inject.hostname }),
                            ("Username",  if self.inject.username.is_empty() { "— (not set)" } else { &self.inject.username }),
                            ("Password",  if self.inject.password.is_empty() { "— (not set)" } else { "••••••• (set)" }),
                            ("Timezone",  tz_display),
                            ("Storage",   storage_display),
                        ];
                        for (k, v) in rows {
                            let val_col = if v.starts_with("— (not set)") { AMBER } else { TEXT };
                            ui.horizontal(|ui| {
                                ui.label(
                                    RichText::new(format!("{k:12}"))
                                        .size(13.0)
                                        .color(MUTED)
                                        .monospace(),
                                );
                                ui.label(RichText::new(*v).size(13.0).color(val_col));
                            });
                        }
                        if !self.inject.ssh_keys.trim().is_empty() {
                            let key_count = self.inject.ssh_keys.lines().filter(|l| !l.trim().is_empty()).count();
                            ui.horizontal(|ui| {
                                ui.label(RichText::new(format!("{:12}", "SSH Keys")).size(13.0).color(MUTED).monospace());
                                ui.label(RichText::new(format!("{key_count} authorized key(s)")).size(13.0).color(GREEN));
                            });
                        }
                        if self.inject.docker {
                            ui.horizontal(|ui| {
                                ui.label(RichText::new(format!("{:12}", "Docker")).size(13.0).color(MUTED).monospace());
                                ui.label(RichText::new("Docker CE will be installed").size(13.0).color(TEXT));
                            });
                        }
                        if !self.inject.packages.trim().is_empty() {
                            let pkg_count = self.inject.packages.lines().filter(|l| !l.trim().is_empty()).count();
                            ui.horizontal(|ui| {
                                ui.label(RichText::new(format!("{:12}", "Packages")).size(13.0).color(MUTED).monospace());
                                ui.label(RichText::new(format!("{pkg_count} extra package(s)")).size(13.0).color(TEXT));
                            });
                        }
                        if self.inject.no_user_interaction {
                            ui.add_space(6.0);
                            ui.label(
                                RichText::new("⚡ Fully unattended — installer will wipe the target disk without any prompts")
                                    .size(12.0)
                                    .color(AMBER),
                            );
                        }
                    });

                // Disk-wipe warning — always visible before the action button
                ui.add_space(10.0);
                Frame::new()
                    .fill(Color32::from_rgb(35, 18, 10))
                    .stroke(Stroke::new(1.0, Color32::from_rgb(140, 70, 20)))
                    .corner_radius(6.0f32)
                    .inner_margin(12.0f32)
                    .show(ui, |ui| {
                        ui.label(
                            RichText::new(
                                "⚠  Booting from this ISO will ERASE the target machine's disk immediately — \
                                 no confirmation screen.  Make sure you're booting the right machine.",
                            )
                            .size(13.0)
                            .color(Color32::from_rgb(230, 160, 60)),
                        );
                    });

                ui.add_space(16.0);
                section(ui, "Output");

                ui.columns(2, |cols| {
                    cols[0].vertical(|ui| {
                        lbl(ui, "Output Directory");
                        ui.horizontal(|ui| {
                            ui.add_enabled(
                                !running,
                                egui::TextEdit::singleline(&mut self.inject.output_dir)
                                    .desired_width(f32::INFINITY)
                                    .min_size(Vec2::new(0.0, 40.0)),
                            );
                            if ui
                                .add_enabled(
                                    !running,
                                    egui::Button::new("📂")
                                        .fill(SURFACE)
                                        .stroke(Stroke::new(1.0, BORDER))
                                        .min_size(Vec2::new(40.0, 40.0)),
                                )
                                .clicked()
                            {
                                worker::pick_folder(
                                    PickTarget::InjectOutputDir,
                                    self.tx.clone(),
                                );
                            }
                        });
                    });
                    cols[1].vertical(|ui| {
                        lbl(ui, "Filename");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.out_name)
                                .hint_text("forgeiso-local.iso")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 40.0)),
                        );
                    });
                });

                ui.add_space(10.0);

                // "Fully unattended" — was defaulting to true (dangerous); now
                // explicit opt-in.  Shown prominently with a risk note.
                let unattended_before = self.inject.no_user_interaction;
                ui.add_enabled(
                    !running,
                    egui::Checkbox::new(
                        &mut self.inject.no_user_interaction,
                        "Fully unattended install  (disk is wiped without any prompts)",
                    ),
                );
                if self.inject.no_user_interaction && !unattended_before {
                    // User just checked it — log a reminder in the status bar
                    self.set_status(StatusMsg::err(
                        "Unattended mode on — booting this ISO WILL wipe the disk immediately",
                    ));
                }
                if !self.inject.no_user_interaction {
                    ui.add_space(2.0);
                    ui.label(
                        RichText::new(
                            "ℹ  Without this, the OS installer shows its normal setup screens.",
                        )
                        .size(11.0)
                        .color(Color32::from_rgb(55, 63, 74)),
                    );
                }

                // SHA-256 source verification — collapsed by default (advanced / rarely used)
                let sha_invalid = {
                    let s = self.inject.expected_sha256.trim();
                    !s.is_empty()
                        && (s.len() != 64 || !s.chars().all(|c| c.is_ascii_hexdigit()))
                };
                ui.add_space(8.0);
                egui::CollapsingHeader::new(
                    RichText::new("🔑  Verify source checksum before injecting  (optional)")
                        .size(12.0)
                        .color(if sha_invalid { AMBER } else { MUTED }),
                )
                .default_open(!self.inject.expected_sha256.is_empty())
                .show(ui, |ui| {
                    ui.add_space(4.0);
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.expected_sha256)
                            .hint_text("64-character hex SHA-256  (leave blank to skip)")
                            .desired_width(ui.available_width()),
                    );
                    if sha_invalid {
                        ui.label(
                            RichText::new("⚠  Must be exactly 64 hex characters.")
                                .size(12.0)
                                .color(RED),
                        );
                    } else {
                        ui.label(
                            RichText::new(
                                "ForgeISO will abort if the downloaded source doesn't match.",
                            )
                            .size(11.0)
                            .color(MUTED),
                        );
                    }
                });

                ui.add_space(20.0);

                let out_empty = self.inject.output_dir.trim().is_empty();
                if out_empty {
                    ui.label(
                        RichText::new("Output directory is required.")
                            .size(13.0)
                            .color(AMBER),
                    );
                    ui.add_space(6.0);
                }

                let can = !out_empty && !sha_invalid && !running;

                // Primary action button (full width).
                let btn_label = if running {
                    "⏳  Building your ISO…"
                } else {
                    "🔨  Build My ISO"
                };
                let fill = if can { ACCENT } else { TAB_ACTIVE };
                let text_col = if can { Color32::WHITE } else { MUTED };
                if ui
                    .add_enabled(
                        can,
                        egui::Button::new(
                            RichText::new(btn_label).size(15.0).strong().color(text_col),
                        )
                        .fill(fill)
                        .stroke(Stroke::new(1.0, if can { ACCENT } else { BORDER }))
                        .corner_radius(8.0f32)
                        .min_size(Vec2::new(ui.available_width(), 50.0)),
                    )
                    .clicked()
                {
                    do_inject = true;
                }

                // ── Result ────────────────────────────────────────────────
                if let Some(r) = self.inject_result.clone() {
                    ui.add_space(16.0);

                    // Success banner
                    Frame::new()
                        .fill(Color32::from_rgb(10, 30, 16))
                        .stroke(Stroke::new(2.0, Color32::from_rgb(40, 130, 60)))
                        .corner_radius(10.0f32)
                        .inner_margin(18.0f32)
                        .show(ui, |ui| {
                            ui.label(
                                RichText::new("✅  Your custom ISO is ready!")
                                    .size(20.0)
                                    .strong()
                                    .color(GREEN),
                            );
                            ui.add_space(4.0);
                            ui.label(
                                RichText::new(
                                    "Copy to USB, boot from it — OS installs automatically \
                                     with your exact settings. No keyboard needed.",
                                )
                                .size(13.0)
                                .color(MUTED),
                            );

                            ui.add_space(12.0);

                            // Output path(s)
                            for a in &r.artifacts {
                                let path_str = a.to_string_lossy();
                                let avail = ui.available_width();
                                ui.label(
                                    RichText::new("Saved to:")
                                        .size(12.0)
                                        .color(MUTED),
                                );
                                ui.add_space(3.0);
                                ui.horizontal(|ui| {
                                    ui.add(
                                        egui::TextEdit::singleline(
                                            &mut path_str.as_ref().to_string(),
                                        )
                                        .desired_width((avail - 154.0).max(50.0))
                                        .interactive(false)
                                        .font(egui::FontId::monospace(13.0)),
                                    );
                                    if ui
                                        .add(
                                            egui::Button::new(
                                                RichText::new("📋 Copy").size(13.0).color(TEXT),
                                            )
                                            .fill(SURFACE)
                                            .stroke(Stroke::new(1.0, BORDER))
                                            .corner_radius(4.0f32)
                                            .min_size(Vec2::new(76.0, 34.0)),
                                        )
                                        .clicked()
                                    {
                                        ui.ctx().copy_text(path_str.clone().into_owned());
                                    }
                                    if ui
                                        .add(
                                            egui::Button::new(
                                                RichText::new("📂 Open").size(13.0).color(TEXT),
                                            )
                                            .fill(SURFACE)
                                            .stroke(Stroke::new(1.0, BORDER))
                                            .corner_radius(4.0f32)
                                            .min_size(Vec2::new(76.0, 34.0)),
                                        )
                                        .clicked()
                                    {
                                        if let Some(dir) = a.parent() {
                                            if std::process::Command::new("xdg-open")
                                                .arg(dir)
                                                .spawn()
                                                .is_err()
                                            {
                                                self.set_status(StatusMsg::err(
                                                    "xdg-open failed — open the folder manually",
                                                ));
                                            }
                                        }
                                    }
                                });
                            }

                            // SHA-256 — prominent section
                            ui.add_space(12.0);
                            ui.add(egui::Separator::default().horizontal().spacing(0.0));
                            ui.add_space(8.0);

                            ui.horizontal(|ui| {
                                ui.label(
                                    RichText::new("SHA-256")
                                        .size(13.0)
                                        .strong()
                                        .color(TEXT),
                                );
                                ui.add_space(6.0);
                                ui.label(
                                    RichText::new("(verify this matches before flashing)")
                                        .size(12.0)
                                        .color(MUTED),
                                );
                            });
                            ui.add_space(4.0);

                            match &self.inject_sha256.clone() {
                                Some(hash) if hash.is_empty() => {
                                    // Hash failed (file deleted, permission error)
                                    ui.label(
                                        RichText::new("SHA-256 unavailable — ISO may have been moved or deleted")
                                            .size(12.0)
                                            .color(AMBER),
                                    );
                                }
                                Some(hash) => {
                                    let avail = ui.available_width();
                                    ui.horizontal(|ui| {
                                        ui.add(
                                            egui::TextEdit::singleline(&mut hash.clone())
                                                .desired_width((avail - 86.0).max(50.0))
                                                .interactive(false)
                                                .font(egui::FontId::monospace(12.0)),
                                        );
                                        if ui
                                            .add(
                                                egui::Button::new(
                                                    RichText::new("📋 Copy")
                                                        .size(13.0)
                                                        .color(TEXT),
                                                )
                                                .fill(SURFACE)
                                                .stroke(Stroke::new(1.0, BORDER))
                                                .corner_radius(4.0f32)
                                                .min_size(Vec2::new(78.0, 32.0)),
                                            )
                                            .clicked()
                                        {
                                            ui.ctx().copy_text(hash.clone());
                                        }
                                    });
                                }
                                None => {
                                    ui.horizontal(|ui| {
                                        ui.spinner();
                                        ui.add_space(6.0);
                                        ui.label(
                                            RichText::new("Computing checksum…")
                                                .size(13.0)
                                                .color(MUTED),
                                        );
                                    });
                                }
                            }

                            // Flash instructions — compact
                            ui.add_space(12.0);
                            ui.add(egui::Separator::default().horizontal().spacing(0.0));
                            ui.add_space(6.0);
                            ui.label(
                                RichText::new("How to flash to USB:")
                                    .size(12.0)
                                    .strong()
                                    .color(MUTED),
                            );
                            ui.add_space(3.0);
                            Frame::new()
                                .fill(Color32::from_rgb(8, 12, 18))
                                .corner_radius(4.0f32)
                                .inner_margin(8.0f32)
                                .show(ui, |ui| {
                                    ui.label(
                                        RichText::new(
                                            "Linux:  sudo dd if=<iso> of=/dev/sdX bs=4M status=progress && sync\n\
                                             macOS:  sudo dd if=<iso> of=/dev/rdiskN bs=4m\n\
                                             GUI:    Balena Etcher — free, cross-platform",
                                        )
                                        .size(12.0)
                                        .monospace()
                                        .color(Color32::from_rgb(90, 185, 110)),
                                    );
                                });

                            // Navigate to results step
                            ui.add_space(12.0);
                            if action_btn(ui, "→  View Results & Verify", true) {
                                self.app_step = AppStep::Check;
                            }
                        });
                }

                ui.add_space(16.0);
            });
        if do_inject {
            self.spawn_inject();
        }
    }

    fn show_inject_advanced(&mut self, ui: &mut Ui, running: bool) {
        let full_w = ui.available_width();
        let col_w = (full_w - 20.0) / 2.0;

        // ── Network ───────────────────────────────────────────────────────
        section(ui, "Network");
        egui::Grid::new("adv_net_grid")
            .num_columns(2)
            .min_col_width(col_w)
            .spacing([20.0, 16.0])
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    lbl(ui, "DNS Servers  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.dns_servers)
                            .hint_text("8.8.8.8\n1.1.1.1")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "NTP Servers  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.ntp_servers)
                            .hint_text("pool.ntp.org")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "Static IP / CIDR  (blank = DHCP)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.static_ip)
                            .hint_text("192.168.1.10/24")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Gateway");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.gateway)
                            .hint_text("192.168.1.1")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "HTTP Proxy");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.http_proxy)
                            .hint_text("http://proxy:3128")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "HTTPS Proxy");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.https_proxy)
                            .hint_text("http://proxy:3128")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
            });
        lbl(ui, "No-proxy (comma-separated)");
        ui.add_enabled(
            !running,
            egui::TextEdit::singleline(&mut self.inject.no_proxy)
                .hint_text("localhost,127.0.0.1,.internal")
                .desired_width(full_w),
        );

        rule(ui);

        // ── System (wallpaper only — tz/locale/keyboard are in step 2) ──────
        section(ui, "System");
        ui.columns(2, |cols| {
            cols[0].vertical(|ui| {
                lbl(ui, "Wallpaper  (image path)");
                ui.horizontal(|ui| {
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.wallpaper_path)
                            .desired_width(f32::INFINITY),
                    );
                    if ui
                        .add_enabled(
                            !running,
                            egui::Button::new("📂")
                                .fill(SURFACE)
                                .stroke(Stroke::new(1.0, BORDER))
                                .min_size(Vec2::new(32.0, 30.0)),
                        )
                        .clicked()
                    {
                        worker::pick_file(PickTarget::InjectWallpaper, self.tx.clone());
                    }
                });
            });
            cols[1].vertical(|ui| {
                lbl(ui, "Volume Label  (blank = keep original, max 32 chars)");
                ui.add_enabled(
                    !running,
                    egui::TextEdit::singleline(&mut self.inject.output_label)
                        .hint_text("MY-UBUNTU  (max 32 chars)")
                        .desired_width(f32::INFINITY),
                );
            });
        });

        rule(ui);

        // ── Packages & Repositories ────────────────────────────────────────
        // Right column content is entirely determined by the selected distro.
        let distro_str = self.inject.distro.clone();
        let is_apt = matches!(distro_str.as_str(), "ubuntu" | "mint");
        let is_fedora = distro_str == "fedora";
        let is_arch = distro_str == "arch";

        let repo_section_label = match distro_str.as_str() {
            "fedora" => "Packages & DNF Repositories",
            "arch" => "Packages & Pacman Repositories",
            _ => "Packages & APT Repositories",
        };
        section(ui, repo_section_label);

        egui::Grid::new("adv_pkg_repos_grid")
            .num_columns(2)
            .min_col_width(col_w)
            .spacing([20.0, 16.0])
            .show(ui, |ui| {
                // ── Left: Extra Packages (always visible) ──────────────
                ui.vertical(|ui| {
                    lbl(ui, "Extra Packages  (one per line)");
                    let pkg_hint = match distro_str.as_str() {
                        "fedora" => "curl\ngit\nvim\nhtop\nunzip\nrsync\nwget\njq\nnet-tools\nbash-completion",
                        "arch"   => "curl\ngit\nvim\nhtop\nunzip\nrsync\nwget\njq\nnet-tools\nbash-completion",
                        _        => "curl\ngit\nvim\nhtop\nunzip\nrsync\nwget\njq\nnet-tools\nbash-completion",
                    };
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.packages)
                            .hint_text(pkg_hint)
                            .desired_width(f32::INFINITY)
                            .desired_rows(6),
                    );
                });

                // ── Right: distro-specific mirror + repo pickers ───────
                ui.vertical(|ui| {
                    // ── APT (Ubuntu / Linux Mint) ──────────────────────
                    if is_apt {
                        let mirror_label = if distro_str == "mint" {
                            "APT Mirror  (Linux Mint / Ubuntu — click to preset)"
                        } else {
                            "APT Mirror  (Ubuntu — click to preset)"
                        };
                        lbl(ui, mirror_label);
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            ui.style_mut().spacing.item_spacing.y = 4.0;
                            let apt_mirrors: &[(&str, &str)] = if distro_str == "mint" {
                                &[
                                    ("official",    "http://archive.ubuntu.com/ubuntu"),
                                    ("US",          "http://us.archive.ubuntu.com/ubuntu"),
                                    ("kernel.org",  "http://mirrors.edge.kernel.org/ubuntu"),
                                    ("MIT",         "http://mirrors.mit.edu/ubuntu"),
                                    ("OSUOSL",      "http://ftp.osuosl.org/pub/ubuntu"),
                                    ("Mint pkgs",   "http://packages.linuxmint.com"),
                                ]
                            } else {
                                &[
                                    ("official",    "http://archive.ubuntu.com/ubuntu"),
                                    ("US",          "http://us.archive.ubuntu.com/ubuntu"),
                                    ("kernel.org",  "http://mirrors.edge.kernel.org/ubuntu"),
                                    ("MIT",         "http://mirrors.mit.edu/ubuntu"),
                                    ("OSUOSL",      "http://ftp.osuosl.org/pub/ubuntu"),
                                    ("Fastly CDN",  "http://mirror.lstn.net/ubuntu"),
                                ]
                            };
                            for (label, url) in apt_mirrors {
                                chip_btn(ui, label, url, &mut self.inject.apt_mirror, !running);
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.apt_mirror)
                                .hint_text("http://archive.ubuntu.com/ubuntu")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 34.0)),
                        );
                        ui.add_space(8.0);

                        let repo_btn_label = if distro_str == "mint" {
                            "Extra APT Repos  (Ubuntu/Mint — click to add)"
                        } else {
                            "Extra APT Repos  (click to add)"
                        };
                        lbl(ui, repo_btn_label);
                        // Ubuntu and Mint share the same APT ecosystem — identical repo presets.
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            for (label, repo) in [
                                ("universe",      "deb http://archive.ubuntu.com/ubuntu noble universe"),
                                ("multiverse",    "deb http://archive.ubuntu.com/ubuntu noble multiverse"),
                                ("restricted",    "deb http://archive.ubuntu.com/ubuntu noble restricted"),
                                ("Docker CE",     "deb [arch=amd64 signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu noble stable"),
                                ("GitHub CLI",    "deb [arch=amd64 signed-by=/usr/share/keyrings/githubcli-archive-keyring.gpg] https://cli.github.com/packages stable main"),
                                ("HashiCorp",     "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com noble main"),
                                ("NodeSource 20", "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_20.x nodistro main"),
                                ("MongoDB 7",     "deb [signed-by=/usr/share/keyrings/mongodb-server-7.0.gpg] https://repo.mongodb.org/apt/ubuntu noble/mongodb-org/7.0 multiverse"),
                                ("Grafana",       "deb [signed-by=/usr/share/keyrings/grafana.key] https://apt.grafana.com stable main"),
                                ("ppa:ondrej/php","ppa:ondrej/php"),
                            ] {
                                let already = self.inject.apt_repos.contains(repo);
                                if ui.add_enabled(
                                    !running,
                                    egui::Button::new(
                                        RichText::new(label)
                                            .size(12.0)
                                            .color(if already { MUTED } else { TEXT }),
                                    )
                                    .fill(if already { Color32::from_rgb(18, 22, 28) } else { SURFACE })
                                    .stroke(Stroke::new(1.0, BORDER))
                                    .corner_radius(4.0f32)
                                    .min_size(Vec2::new(0.0, 26.0)),
                                ).on_hover_text(repo).clicked() && !already {
                                    if !self.inject.apt_repos.is_empty()
                                        && !self.inject.apt_repos.ends_with('\n')
                                    {
                                        self.inject.apt_repos.push('\n');
                                    }
                                    self.inject.apt_repos.push_str(repo);
                                }
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::multiline(&mut self.inject.apt_repos)
                                .hint_text("ppa:ondrej/php\ndeb http://archive.ubuntu.com/ubuntu noble universe")
                                .desired_width(f32::INFINITY)
                                .desired_rows(4),
                        );
                    }

                    // ── DNF (Fedora) ───────────────────────────────────
                    if is_fedora {
                        lbl(ui, "DNF Mirror  (Fedora — click to select)");
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            ui.style_mut().spacing.item_spacing.y = 4.0;
                            for (label, url) in [
                                ("official",  "https://download.fedoraproject.org/pub/fedora/linux"),
                                ("MIT",       "https://mirrors.mit.edu/fedora/linux"),
                                ("OSUOSL",    "https://ftp.osuosl.org/pub/fedora/linux"),
                                ("kernel.org","https://mirrors.kernel.org/fedora"),
                                ("Rackspace", "https://mirror.rackspace.com/fedora"),
                                ("Nerd.dk",   "https://mirror.nerd.dk/fedora-linux"),
                            ] {
                                chip_btn(ui, label, url, &mut self.inject.dnf_mirror, !running);
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.dnf_mirror)
                                .hint_text("https://download.fedoraproject.org/pub/fedora/linux")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 34.0)),
                        );
                        ui.add_space(8.0);
                        lbl(ui, "Extra DNF Repos  (Fedora — click to add)");
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            for (label, repo) in [
                                ("EPEL",              "https://dl.fedoraproject.org/pub/epel/epel-release-latest-${releasever}.noarch.rpm"),
                                ("RPMFusion Free",    "https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-${releasever}.noarch.rpm"),
                                ("RPMFusion NonFree", "https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-${releasever}.noarch.rpm"),
                                ("Docker CE",         "https://download.docker.com/linux/fedora/docker-ce.repo"),
                                ("HashiCorp",         "https://rpm.releases.hashicorp.com/fedora/hashicorp.repo"),
                                ("GitHub CLI",        "https://cli.github.com/packages/rpm/gh-cli.repo"),
                                ("VS Code",           "https://packages.microsoft.com/yumrepos/vscode"),
                                ("Google Chrome",     "https://dl.google.com/linux/chrome/rpm/stable/x86_64"),
                                ("NodeSource 20",     "https://rpm.nodesource.com/pub_20.x/nodistro/nodejs/${arch}/nodesource-release-nodistro-1.noarch.rpm"),
                                ("Grafana",           "https://rpm.grafana.com/oss/release"),
                                ("Kubernetes",        "https://pkgs.k8s.io/core:/stable:/v1.30/rpm/"),
                                ("Tailscale",         "https://pkgs.tailscale.com/stable/fedora/${releasever}/tailscale.repo"),
                            ] {
                                let already = self.inject.dnf_repos.contains(repo);
                                if ui.add_enabled(
                                    !running,
                                    egui::Button::new(
                                        RichText::new(label)
                                            .size(12.0)
                                            .color(if already { MUTED } else { TEXT }),
                                    )
                                    .fill(if already { Color32::from_rgb(18, 22, 28) } else { SURFACE })
                                    .stroke(Stroke::new(1.0, BORDER))
                                    .corner_radius(4.0f32)
                                    .min_size(Vec2::new(0.0, 26.0)),
                                ).on_hover_text(repo).clicked() && !already {
                                    if !self.inject.dnf_repos.is_empty()
                                        && !self.inject.dnf_repos.ends_with('\n')
                                    {
                                        self.inject.dnf_repos.push('\n');
                                    }
                                    self.inject.dnf_repos.push_str(repo);
                                }
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::multiline(&mut self.inject.dnf_repos)
                                .hint_text("https://dl.fedoraproject.org/pub/epel/epel-release-latest-${releasever}.noarch.rpm")
                                .desired_width(f32::INFINITY)
                                .desired_rows(4),
                        );
                    }

                    // ── Pacman (Arch Linux) ────────────────────────────
                    if is_arch {
                        lbl(ui, "Pacman Mirror  (Arch Linux — click to select)");
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            ui.style_mut().spacing.item_spacing.y = 4.0;
                            for (label, url) in [
                                ("Cloudflare", "https://cloudflaremirrors.com/archlinux"),
                                ("MIT",        "https://mirrors.mit.edu/archlinux"),
                                ("OSUOSL",     "https://ftp.osuosl.org/pub/archlinux"),
                                ("kernel.org", "https://mirrors.edge.kernel.org/archlinux"),
                                ("Rackspace",  "https://mirror.rackspace.com/archlinux"),
                                ("Xtom",       "https://mirror.xtom.com/archlinux"),
                            ] {
                                chip_btn(ui, label, url, &mut self.inject.pacman_mirror, !running);
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::singleline(&mut self.inject.pacman_mirror)
                                .hint_text("https://cloudflaremirrors.com/archlinux")
                                .desired_width(f32::INFINITY)
                                .min_size(Vec2::new(0.0, 34.0)),
                        );
                        ui.add_space(8.0);
                        lbl(ui, "Extra Pacman Repos  (Arch — Server = lines, click to add)");
                        ui.horizontal_wrapped(|ui| {
                            ui.style_mut().spacing.item_spacing.x = 6.0;
                            for (label, repo) in [
                                ("Cloudflare",  "Server = https://cloudflaremirrors.com/archlinux/$repo/os/$arch"),
                                ("kernel.org",  "Server = https://mirrors.edge.kernel.org/archlinux/$repo/os/$arch"),
                                ("MIT",         "Server = https://mirrors.mit.edu/archlinux/$repo/os/$arch"),
                                ("OSUOSL",      "Server = https://ftp.osuosl.org/pub/archlinux/$repo/os/$arch"),
                                ("Rackspace",   "Server = https://mirror.rackspace.com/archlinux/$repo/os/$arch"),
                                ("Xtom",        "Server = https://mirror.xtom.com/archlinux/$repo/os/$arch"),
                                ("Chaotic-AUR", "Server = https://cdn-mirror.chaotic.cx/$repo/$arch"),
                                ("ArchLinuxCN", "Server = https://repo.archlinuxcn.org/$arch"),
                                ("BlackArch",   "Server = https://blackarch.org/blackarch/$repo/os/$arch"),
                                ("Arch4Edu",    "Server = https://mirrors.tuna.tsinghua.edu.cn/arch4edu/$arch"),
                            ] {
                                let already = self.inject.pacman_repos.contains(repo);
                                if ui.add_enabled(
                                    !running,
                                    egui::Button::new(
                                        RichText::new(label)
                                            .size(12.0)
                                            .color(if already { MUTED } else { TEXT }),
                                    )
                                    .fill(if already { Color32::from_rgb(18, 22, 28) } else { SURFACE })
                                    .stroke(Stroke::new(1.0, BORDER))
                                    .corner_radius(4.0f32)
                                    .min_size(Vec2::new(0.0, 26.0)),
                                ).on_hover_text(repo).clicked() && !already {
                                    if !self.inject.pacman_repos.is_empty()
                                        && !self.inject.pacman_repos.ends_with('\n')
                                    {
                                        self.inject.pacman_repos.push('\n');
                                    }
                                    self.inject.pacman_repos.push_str(repo);
                                }
                            }
                        });
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::multiline(&mut self.inject.pacman_repos)
                                .hint_text("Server = https://cloudflaremirrors.com/archlinux/$repo/os/$arch")
                                .desired_width(f32::INFINITY)
                                .desired_rows(4),
                        );
                    }
                });
                ui.end_row();
            });

        rule(ui);

        // ── Commands ──────────────────────────────────────────────────────
        section(ui, "Run Commands");
        egui::Grid::new("adv_cmd_grid")
            .num_columns(2)
            .min_col_width(col_w)
            .spacing([20.0, 16.0])
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    lbl(ui, "Early Commands  (run before packages)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.run_commands)
                            .hint_text("apt-get update -qq")
                            .desired_width(f32::INFINITY)
                            .desired_rows(4),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Late Commands  (run at end of install)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.late_commands)
                            .hint_text("systemctl enable myservice")
                            .desired_width(f32::INFINITY)
                            .desired_rows(4),
                    );
                });
                ui.end_row();
            });

        rule(ui);

        // ── Services & Containers ─────────────────────────────────────────
        section(ui, "Services & Containers");
        egui::Grid::new("adv_svc_grid")
            .num_columns(2)
            .min_col_width(col_w)
            .spacing([20.0, 16.0])
            .show(ui, |ui| {
                ui.vertical(|ui| {
                    lbl(ui, "Enable Services  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.enable_services)
                            .hint_text("docker\nssh")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Disable Services  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.disable_services)
                            .hint_text("snapd")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "User Groups  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.user_groups)
                            .hint_text("docker\nsudo")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.vertical(|ui| {
                    ui.add_space(16.0);
                    ui.add_enabled(
                        !running,
                        egui::Checkbox::new(&mut self.inject.docker, "Install Docker"),
                    );
                    ui.add_space(4.0);
                    ui.add_enabled(
                        !running,
                        egui::Checkbox::new(&mut self.inject.podman, "Install Podman"),
                    );
                    ui.add_space(4.0);
                    ui.add_enabled(
                        !running,
                        egui::Checkbox::new(&mut self.inject.sudo_nopasswd, "Passwordless sudo"),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "Login Shell  (blank = system default)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.user_shell)
                            .hint_text("/bin/bash")
                            .desired_width(f32::INFINITY),
                    );
                    ui.add_space(6.0);
                    lbl(
                        ui,
                        "Sudoers Commands  (one per line, ignored if passwordless sudo is set)",
                    );
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.sudo_commands)
                            .hint_text("/usr/bin/apt\n/usr/sbin/reboot")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Docker Group Users  (one per line)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::multiline(&mut self.inject.docker_users)
                            .hint_text("admin\ndevuser")
                            .desired_width(f32::INFINITY)
                            .desired_rows(3),
                    );
                });
                ui.end_row();
            });

        rule(ui);

        // ── Firewall ──────────────────────────────────────────────────────
        section(ui, "Firewall  (UFW on Ubuntu/Mint · firewalld on Fedora)");
        ui.horizontal(|ui| {
            ui.add_enabled(
                !running,
                egui::Checkbox::new(&mut self.inject.firewall_enabled, "Enable firewall"),
            );
            if self.inject.firewall_enabled {
                ui.add_space(16.0);
                lbl(ui, "Default policy:");
                ui.add_space(4.0);
                ui.add_enabled_ui(!running, |ui| {
                    let cur = if self.inject.firewall_policy.is_empty() {
                        "deny"
                    } else {
                        &self.inject.firewall_policy
                    };
                    egui::ComboBox::from_id_salt("fw_policy_combo")
                        .selected_text(cur)
                        .width(160.0)
                        .show_ui(ui, |ui| {
                            for (label, val) in [
                                ("deny  (recommended)", "deny"),
                                ("allow", "allow"),
                                ("reject", "reject"),
                            ] {
                                if ui
                                    .selectable_label(self.inject.firewall_policy == val, label)
                                    .clicked()
                                {
                                    self.inject.firewall_policy = val.to_owned();
                                }
                            }
                        });
                });
            }
        });
        if self.inject.firewall_enabled {
            ui.add_space(6.0);
            egui::Grid::new("adv_fw_grid")
                .num_columns(2)
                .spacing([20.0, 16.0])
                .show(ui, |ui| {
                    ui.vertical(|ui| {
                        lbl(ui, "Allow Ports  (one per line, e.g. 22/tcp)");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::multiline(&mut self.inject.allow_ports)
                                .hint_text("22/tcp\n443")
                                .desired_width(f32::INFINITY)
                                .desired_rows(4),
                        );
                    });
                    ui.vertical(|ui| {
                        lbl(ui, "Deny Ports  (one per line)");
                        ui.add_enabled(
                            !running,
                            egui::TextEdit::multiline(&mut self.inject.deny_ports)
                                .hint_text("23")
                                .desired_width(f32::INFINITY)
                                .desired_rows(4),
                        );
                    });
                    ui.end_row();
                });
        }

        rule(ui);

        // ── Boot & Storage ────────────────────────────────────────────────
        section(ui, "Boot & Storage");
        egui::Grid::new("adv_boot_grid")
            .num_columns(2)
            .min_col_width(col_w)
            .spacing([20.0, 16.0])
            .show(ui, |ui| {
                // Storage layout — moved here from main form (it is an advanced setting)
                ui.vertical(|ui| {
                    lbl(ui, "Storage Layout");
                    let storage_presets: &[(&str, &str)] = &[
                        ("Direct (default)", "direct"),
                        ("LVM", "lvm"),
                        ("LVM + LUKS", "lvm+luks"),
                        ("ZFS (experimental)", "zfs"),
                    ];
                    combo_and_text(
                        ui,
                        "storage_combo",
                        storage_presets,
                        &mut self.inject.storage_layout,
                        "direct",
                        !running,
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "GRUB Timeout  (seconds)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.grub_timeout)
                            .hint_text("5")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "GRUB Default Entry");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.grub_default)
                            .hint_text("Ubuntu")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "Extra Kernel Cmdline Args");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.grub_cmdline)
                            .hint_text("quiet splash")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Swap Size  (MB, blank = none)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.swap_size_mb)
                            .hint_text("2048")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
                ui.vertical(|ui| {
                    lbl(ui, "Swap File Path  (blank = /swapfile)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.swap_filename)
                            .hint_text("/swapfile")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.vertical(|ui| {
                    lbl(ui, "Swappiness  (0–100, blank = system default)");
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.inject.swap_swappiness)
                            .hint_text("10")
                            .desired_width(f32::INFINITY),
                    );
                });
                ui.end_row();
            });
        lbl(ui, "Custom Mounts  (fstab entries, one per line)");
        ui.add_enabled(
            !running,
            egui::TextEdit::multiline(&mut self.inject.mounts)
                .hint_text("/dev/sdb1  /data  ext4  defaults  0  2")
                .desired_width(full_w)
                .desired_rows(3),
        );
        ui.add_space(4.0);

        // ── Encryption ────────────────────────────────────────────────────────
        rule(ui);
        section(ui, "Disk Encryption  (LUKS, Ubuntu only)");
        ui.horizontal(|ui| {
            ui.add_enabled(
                !running,
                egui::Checkbox::new(&mut self.inject.encrypt, "Enable LUKS encryption"),
            );
        });
        if self.inject.encrypt {
            ui.add_space(4.0);
            lbl(
                ui,
                "LUKS Passphrase  (written to cloud-init — treat as low-security pre-boot)",
            );
            ui.add_enabled(
                !running,
                egui::TextEdit::singleline(&mut self.inject.encrypt_passphrase)
                    .password(true)
                    .hint_text("passphrase")
                    .desired_width(full_w),
            );
        }
        ui.add_space(4.0);

        lbl(ui, "Sysctl  (key=value, one per line)");
        ui.add_enabled(
            !running,
            egui::TextEdit::multiline(&mut self.inject.sysctl_pairs)
                .hint_text("net.ipv4.ip_forward=1\nvm.swappiness=10")
                .desired_width(full_w)
                .desired_rows(4),
        );

        ui.add_space(6.0);
    }

    fn show_verify(&mut self, ui: &mut Ui) {
        let running = self.job_running;
        let mut do_verify = false;
        let mut do_9660 = false;

        egui::ScrollArea::vertical()
            .id_salt("verify_scroll")
            .show(ui, |ui| {
                ui.add_space(12.0);

                // ── SHA-256 Checksum ────────────────────────────────────
                ui.label(
                    RichText::new("🔒  Verify Your ISO File")
                        .size(20.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(6.0);
                ui.label(
                    RichText::new(
                        "Checks that your ISO file hasn't been corrupted or tampered with. \
                         After creating your custom ISO, run this to confirm the file is intact.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(10.0);

                lbl(ui, "ISO Path  (local path or URL)");
                ui.horizontal(|ui| {
                    ui.add_enabled(
                        !running,
                        egui::TextEdit::singleline(&mut self.verify.source)
                            .hint_text("/path/to/ubuntu.iso")
                            .desired_width((ui.available_width() - 96.0).max(50.0))
                            .min_size(Vec2::new(0.0, 38.0)),
                    );
                    if browse_btn(ui, !running) {
                        worker::pick_iso(PickTarget::VerifySource, self.tx.clone());
                    }
                });

                ui.add_space(6.0);
                lbl(ui, "SHA256SUMS URL  (optional — auto-detected for Ubuntu)");
                ui.add_enabled(
                    !running,
                    egui::TextEdit::singleline(&mut self.verify.sums_url)
                        .hint_text("https://releases.ubuntu.com/24.04/SHA256SUMS")
                        .desired_width(ui.available_width()),
                );

                ui.add_space(12.0);
                let can_verify = !self.verify.source.trim().is_empty() && !running;
                let verify_lbl = if running && self.job_phase.to_lowercase().contains("verify") {
                    "⏳  Checking…"
                } else {
                    "🔒  Check File Integrity"
                };
                if action_btn(ui, verify_lbl, can_verify) {
                    do_verify = true;
                }

                // ── Verify result ────────────────────────────────────────
                if let Some(r) = self.verify_result.clone() {
                    ui.add_space(12.0);
                    let (fill, border, icon) = if r.matched {
                        (CARD_GREEN_BG, CARD_GREEN_BORDER, "✅")
                    } else {
                        (CARD_RED_BG, CARD_RED_BORDER, "⚠️")
                    };
                    result_box(ui, fill, border, |ui| {
                        let col = if r.matched { GREEN } else { AMBER };
                        ui.label(
                            RichText::new(format!(
                                "{}  {}",
                                icon,
                                if r.matched {
                                    "Checksum Matched"
                                } else {
                                    "Checksum Not Matched"
                                }
                            ))
                            .size(14.0)
                            .strong()
                            .color(col),
                        );
                        ui.add_space(6.0);
                        ui.horizontal(|ui| {
                            ui.label(
                                RichText::new("File:    ")
                                    .size(14.0)
                                    .monospace()
                                    .color(MUTED),
                            );
                            ui.label(
                                RichText::new(&r.filename)
                                    .size(14.0)
                                    .monospace()
                                    .color(TEXT),
                            );
                        });
                        // Expected
                        let exp_display = if r.expected.len() == 64
                            && r.expected.chars().all(|c| c.is_ascii_hexdigit())
                        {
                            format!("{}…", r.expected.chars().take(32).collect::<String>())
                        } else {
                            r.expected.clone()
                        };
                        ui.horizontal(|ui| {
                            ui.label(
                                RichText::new("Expected:")
                                    .size(14.0)
                                    .monospace()
                                    .color(MUTED),
                            );
                            ui.label(
                                RichText::new(exp_display)
                                    .size(14.0)
                                    .monospace()
                                    .color(MUTED),
                            );
                        });
                        // Actual
                        let act_col = if r.matched { GREEN } else { AMBER };
                        ui.horizontal(|ui| {
                            ui.label(
                                RichText::new("Actual:  ")
                                    .size(14.0)
                                    .monospace()
                                    .color(MUTED),
                            );
                            ui.label(
                                RichText::new(format!(
                                    "{}…",
                                    r.actual.chars().take(32).collect::<String>()
                                ))
                                .size(14.0)
                                .monospace()
                                .color(act_col),
                            );
                            if ui
                                .small_button("📋")
                                .on_hover_text("Copy full SHA-256")
                                .clicked()
                            {
                                ui.ctx().copy_text(r.actual.clone());
                            }
                        });
                    });
                }

                rule(ui);

                // ── ISO-9660 Validation ──────────────────────────────────
                ui.label(
                    RichText::new("ISO-9660 Structure Validation")
                        .size(17.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(4.0);
                ui.label(
                    RichText::new(
                        "Checks that the ISO has a valid ISO-9660 filesystem header. \
                         Uses the same source path as checksum verification above.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(10.0);
                let can_9660 = !self.verify.source.trim().is_empty() && !running;
                let iso_lbl = if running && self.job_phase.to_lowercase().contains("9660") {
                    "⏳  Validating…"
                } else {
                    "Validate ISO-9660"
                };
                if small_btn(ui, iso_lbl, can_9660) {
                    do_9660 = true;
                }

                if let Some(r) = self.iso9660_result.clone() {
                    ui.add_space(8.0);
                    let (fill, border, msg, col) = if r.compliant {
                        (
                            CARD_GREEN_BG,
                            CARD_GREEN_BORDER,
                            "✅  ISO-9660 Compliant",
                            GREEN,
                        )
                    } else {
                        (CARD_RED_BG, CARD_RED_BORDER, "❌  Not Compliant", RED)
                    };
                    result_box(ui, fill, border, |ui| {
                        ui.label(RichText::new(msg).size(13.0).strong().color(col));
                        if let Some(vid) = &r.volume_id {
                            ui.label(
                                RichText::new(format!("Volume ID: {vid}"))
                                    .size(14.0)
                                    .color(MUTED),
                            );
                        }
                        if let Some(err) = &r.error {
                            ui.label(RichText::new(err).size(13.0).color(RED));
                        }
                    });
                }

                ui.add_space(16.0);
            });

        if do_verify {
            self.spawn_verify();
        }
        if do_9660 {
            self.spawn_iso9660();
        }
    }

    fn show_doctor(&mut self, ui: &mut Ui) {
        let running = self.job_running;

        egui::ScrollArea::vertical()
            .id_salt("doctor_scroll")
            .show(ui, |ui| {
                ui.add_space(12.0);

                ui.label(
                    RichText::new("🩺  System Health Check")
                        .size(20.0)
                        .strong()
                        .color(TEXT),
                );
                ui.add_space(6.0);
                ui.label(
                    RichText::new(
                        "Checks that everything ForgeISO needs is installed on this machine. \
                         Run this first if you get errors.",
                    )
                    .size(14.0)
                    .color(MUTED),
                );
                ui.add_space(10.0);

                let lbl = if running {
                    "⏳  Checking…"
                } else {
                    "🩺  Run Health Check"
                };
                if action_btn(ui, lbl, !running) {
                    self.spawn_doctor();
                }

                if let Some(r) = self.doctor_result.clone() {
                    ui.add_space(12.0);
                    let all_ok = r.tooling.values().all(|&ok| ok);
                    let (fill, border) = if all_ok {
                        (CARD_GREEN_BG, CARD_GREEN_BORDER)
                    } else {
                        (CARD_ORANGE_BG, CARD_ORANGE_BORDER)
                    };

                    result_box(ui, fill, border, |ui| {
                        ui.label(
                            RichText::new(if all_ok {
                                "✅  All dependencies satisfied"
                            } else {
                                "⚠️  Some dependencies missing"
                            })
                            .size(13.0)
                            .strong()
                            .color(if all_ok { GREEN } else { AMBER }),
                        );
                        ui.add_space(8.0);
                        egui::Grid::new("doctor_grid")
                            .num_columns(2)
                            .spacing([24.0, 8.0])
                            .striped(true)
                            .show(ui, |ui| {
                                ui.label(RichText::new("Tool").size(14.0).strong().color(MUTED));
                                ui.label(RichText::new("Status").size(14.0).strong().color(MUTED));
                                ui.end_row();
                                for (name, &ok) in &r.tooling {
                                    ui.label(
                                        RichText::new(name).size(15.0).monospace().color(TEXT),
                                    );
                                    let (status_text, status_col) = if ok {
                                        ("✓ OK", GREEN)
                                    } else {
                                        ("✗ Missing", RED)
                                    };
                                    ui.label(
                                        RichText::new(status_text).size(15.0).color(status_col),
                                    );
                                    ui.end_row();
                                }
                            });
                    });

                    if !all_ok {
                        ui.add_space(12.0);
                        ui.label(
                            RichText::new("Install missing tools with:")
                                .size(14.0)
                                .color(MUTED),
                        );
                        ui.add_space(6.0);
                        Frame::new()
                            .fill(PANEL_BG)
                            .stroke(Stroke::new(1.0, BORDER))
                            .inner_margin(12.0f32)
                            .corner_radius(6.0f32)
                            .show(ui, |ui| {
                                ui.label(
                                    RichText::new(
                                        "sudo dnf install xorriso grub2-tools squashfs-tools",
                                    )
                                    .size(15.0)
                                    .monospace()
                                    .color(TEXT),
                                );
                            });
                    }

                    // ── Distro readiness ─────────────────────────────────
                    if !r.distro_readiness.is_empty() {
                        ui.add_space(16.0);
                        ui.label(
                            RichText::new("Distro Support")
                                .size(15.0)
                                .strong()
                                .color(TEXT),
                        );
                        ui.add_space(6.0);
                        egui::Grid::new("distro_readiness_grid")
                            .num_columns(2)
                            .spacing([24.0, 6.0])
                            .striped(true)
                            .show(ui, |ui| {
                                ui.label(RichText::new("Distro").size(13.0).strong().color(MUTED));
                                ui.label(RichText::new("Ready").size(13.0).strong().color(MUTED));
                                ui.end_row();
                                for (distro, &ready) in &r.distro_readiness {
                                    ui.label(
                                        RichText::new(distro).size(14.0).monospace().color(TEXT),
                                    );
                                    let (txt, col) = if ready {
                                        ("✓ Yes", GREEN)
                                    } else {
                                        ("✗ No", RED)
                                    };
                                    ui.label(RichText::new(txt).size(14.0).color(col));
                                    ui.end_row();
                                }
                            });
                    }

                    // ── Warnings ─────────────────────────────────────────
                    if !r.warnings.is_empty() {
                        ui.add_space(12.0);
                        ui.label(RichText::new("Warnings").size(15.0).strong().color(AMBER));
                        ui.add_space(6.0);
                        for warn in &r.warnings {
                            Frame::new()
                                .fill(CARD_AMBER_BG)
                                .stroke(Stroke::new(1.0, AMBER))
                                .corner_radius(6.0f32)
                                .inner_margin(8.0f32)
                                .show(ui, |ui| {
                                    ui.label(
                                        RichText::new(format!("⚠  {warn}")).size(13.0).color(AMBER),
                                    );
                                });
                            ui.add_space(4.0);
                        }
                    }

                    // ── Host info ────────────────────────────────────────
                    ui.add_space(12.0);
                    ui.label(
                        RichText::new(format!(
                            "Host: {} {}  ·  {}",
                            r.host_os, r.host_arch, r.timestamp
                        ))
                        .size(12.0)
                        .color(MUTED),
                    );
                }

                ui.add_space(16.0);
            });
    }
}

// ── eframe::App impl ─────────────────────────────────────────────────────────

impl eframe::App for ForgeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        // Run an initial doctor check on first startup (deferred from new()
        // because spawn_doctor requires &mut self to start the tokio task).
        if self.needs_initial_doctor && !self.job_running {
            self.needs_initial_doctor = false;
            self.spawn_doctor();
        }

        self.drain_messages(ctx);

        // Request repaint while a job is running so progress shows smoothly
        if self.job_running {
            ctx.request_repaint_after(std::time::Duration::from_millis(100));
        }

        self.render_header(ctx);
        self.render_step_bar(ctx);
        self.render_log(ctx);

        egui::CentralPanel::default()
            .frame(
                Frame::new()
                    .fill(BG)
                    .inner_margin(egui::Margin::symmetric(28, 20)),
            )
            .show(ctx, |ui| match self.app_step {
                AppStep::Source => self.show_step_source(ui),
                AppStep::Configure => self.show_step_configure(ui),
                AppStep::Build => self.show_step_build(ui),
                AppStep::Check => self.show_verify(ui),
            });

        // Floating Doctor window
        if self.doctor_open {
            egui::Window::new("🩺  Tool Check")
                .id(egui::Id::new("doctor_win"))
                .default_width(520.0)
                .collapsible(false)
                .show(ctx, |ui| {
                    if ui.button("✕  Close").clicked() {
                        self.doctor_open = false;
                    }
                    ui.add_space(6.0);
                    self.show_doctor(ui);
                });
        }
    }

    fn save(&mut self, storage: &mut dyn eframe::Storage) {
        let state = PersistedState {
            inject: self.inject.clone(),
            verify: self.verify.clone(),
        };
        eframe::set_value(storage, STORAGE_KEY, &state);
    }
}

// ── Build InjectConfig from form state ────────────────────────────────────────

/// Render a fixed-width ASCII progress bar: `|████████────────────| 45%`
fn make_progress_bar(pct: u8) -> String {
    const WIDTH: usize = 24;
    let filled = ((pct as usize) * WIDTH) / 100;
    let empty = WIDTH.saturating_sub(filled);
    format!("|{}{}|", "█".repeat(filled), "─".repeat(empty))
}

fn build_inject_config(inject: &InjectState) -> InjectConfig {
    let distro = match inject.distro.as_str() {
        "fedora" => Some(Distro::Fedora),
        "arch" => Some(Distro::Arch),
        "mint" => Some(Distro::Mint),
        _ => None,
    };

    InjectConfig {
        source: IsoSource::from_raw(&inject.source),
        out_name: inject.out_name.clone(),
        output_label: opt(&inject.output_label),
        autoinstall_yaml: None,
        hostname: opt(&inject.hostname),
        username: opt(&inject.username),
        password: opt(&inject.password),
        realname: opt(&inject.realname),
        ssh: SshConfig {
            authorized_keys: lines(&inject.ssh_keys),
            allow_password_auth: Some(inject.ssh_password_auth),
            install_server: Some(inject.ssh_install_server),
        },
        network: NetworkConfig {
            dns_servers: lines(&inject.dns_servers),
            ntp_servers: lines(&inject.ntp_servers),
        },
        static_ip: opt(&inject.static_ip),
        gateway: opt(&inject.gateway),
        proxy: ProxyConfig {
            http_proxy: opt(&inject.http_proxy),
            https_proxy: opt(&inject.https_proxy),
            no_proxy: inject
                .no_proxy
                .split(',')
                .map(|s| s.trim().to_string())
                .filter(|s| !s.is_empty())
                .collect(),
        },
        timezone: opt(&inject.timezone),
        locale: opt(&inject.locale),
        keyboard_layout: opt(&inject.keyboard_layout),
        storage_layout: opt(&inject.storage_layout),
        apt_mirror: opt(&inject.apt_mirror),
        extra_packages: lines(&inject.packages),
        wallpaper: opt(&inject.wallpaper_path).map(PathBuf::from),
        extra_late_commands: lines(&inject.late_commands),
        no_user_interaction: inject.no_user_interaction,
        user: UserConfig {
            groups: lines(&inject.user_groups),
            shell: opt(&inject.user_shell),
            sudo_nopasswd: inject.sudo_nopasswd,
            sudo_commands: lines(&inject.sudo_commands),
        },
        firewall: FirewallConfig {
            enabled: inject.firewall_enabled,
            default_policy: opt(&inject.firewall_policy),
            allow_ports: lines(&inject.allow_ports),
            deny_ports: lines(&inject.deny_ports),
        },
        enable_services: lines(&inject.enable_services),
        disable_services: lines(&inject.disable_services),
        sysctl: lines(&inject.sysctl_pairs)
            .iter()
            .filter_map(|s| {
                let mut parts = s.splitn(2, '=');
                match (parts.next(), parts.next()) {
                    (Some(k), Some(v)) => Some((k.trim().to_string(), v.trim().to_string())),
                    _ => None,
                }
            })
            .collect(),
        swap: inject
            .swap_size_mb
            .trim()
            .parse::<u32>()
            .ok()
            .filter(|&n| n > 0)
            .map(|size_mb| SwapConfig {
                size_mb,
                filename: opt(&inject.swap_filename),
                swappiness: inject.swap_swappiness.trim().parse::<u8>().ok(),
            }),
        apt_repos: lines(&inject.apt_repos),
        dnf_repos: lines(&inject.dnf_repos),
        dnf_mirror: opt(&inject.dnf_mirror),
        pacman_repos: lines(&inject.pacman_repos),
        pacman_mirror: opt(&inject.pacman_mirror),
        containers: ContainerConfig {
            docker: inject.docker,
            podman: inject.podman,
            docker_users: lines(&inject.docker_users),
        },
        grub: GrubConfig {
            timeout: inject.grub_timeout.trim().parse::<u32>().ok(),
            cmdline_extra: inject
                .grub_cmdline
                .split_whitespace()
                .map(String::from)
                .collect(),
            default_entry: opt(&inject.grub_default),
        },
        encrypt: inject.encrypt,
        encrypt_passphrase: if inject.encrypt && !inject.encrypt_passphrase.is_empty() {
            Some(inject.encrypt_passphrase.clone())
        } else {
            None
        },
        mounts: lines(&inject.mounts),
        run_commands: lines(&inject.run_commands),
        distro,
        expected_sha256: opt(&inject.expected_sha256),
    }
}
