...
fn hit_test_on_close(close: &[WidgetRect], pos: Pos2) -> WidgetHits {
    ...
    match (hit_click, hit_drag) {
        ...
        (Some(hit_click), Some(hit_drag)) => {
            ...
            //
            // Guard against NaN in WidgetRect::rect (e.g. from degenerate column
            // layouts) causing position() to return None: NaN != NaN breaks the
            // PartialEq comparison even when the widget IS in `close`.
            // If we cannot determine the ordering, default to click on top.
            let click_idx = close.iter().position(|w| *w == hit_click);
            let drag_idx = close.iter().position(|w| *w == hit_drag);
            let click_is_on_top_of_drag = match (click_idx, drag_idx) {
                (Some(ci), Some(di)) => di < ci,
                _ => true,
            };
